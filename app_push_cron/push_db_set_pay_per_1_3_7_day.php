...
$DeviceType = "";
	...
	//	( select datediff(ClassOrderEndDate, now()) from ClassOrders where ClassOrderState=1 and ClassProgress=11 and ClassProductID=1 ) as SendMessagePerDay,
/*
$Sql3 = "
      SELECT 
      ...
      FROM Members A 
       ...
       WHERE 
        ...
        A.MemberLevelID=19 
        AND 
        A.MemberID IN ( SELECT MemberID from ClassOrders where ( datediff(ClassOrderEndDate, now())=1 or datediff(ClassOrderEndDate, now())=3 or datediff(ClassOrderEndDate, now())=7 ) and ClassOrderState=1 and ClassProgress=11 AND ClassProductID=1 ) 
		AND A.MemberID=88
";
*/
$Sql3 = "
	SELECT 
		DATEDIFF(E.ClassOrderEndDate, now()) as SendMessagePerDay,
		AES_DECRYPT(UNHEX(A.MemberPhone1), :EncryptionKey) as DecMemberPhone1, 
		AES_DECRYPT(UNHEX(A.MemberPhone2), :EncryptionKey) as DecMemberPhone2, 
		A.*, 
		B.CenterID, 
	...
	FROM Members A 
		...
		inner join Branches C on B.BranchID=C.BranchID 
		inner join BranchGroups D on C.BranchGroupID=D.BranchGroupID 
		inner join ClassOrders E ON E.MemberID=A.MemberID 
	WHERE 
	(
	B.CenterPayType=2 
	OR 
	...
	A.MemberLevelID=19 
	AND 
		( ( DATEDIFF(E.ClassOrderEndDate, now())=1 or datediff(E.ClassOrderEndDate, now())=3 or datediff(E.ClassOrderEndDate, now())=7 ) and E.ClassOrderState=1 and E.ClassProgress=11 AND E.ClassProductID=1 ) 
	GROUP BY E.ClassOrderEndDate, A.MemberID
";
...
while($Row3 = $Stmt3->fetch()) {
		...
		$SendMessagePerDay = $Row3["SendMessagePerDay"];
		...
		$SendTitle="종료알림";
		if($SendMessagePerDay==1) {
			$MsgDate =  date('Y-m-d', strtotime($NowDate. ' +1 day'));
			$SendMessage="$StrDomain $MemberName 님의 수업이 $MsgDate 일자로 종료됩니다.\n당일 결제 시 수업이 불가할 수 있으니 수강 연장을 원하시면 오늘까지 결제 부탁드립니다. ";
		} else if($SendMessagePerDay==3) {
			$MsgDate =  date('Y-m-d', strtotime($NowDate. ' +3 day'));
			$SendMessage="$StrDomain $MemberName 님의 수업이 $MsgDate 일자로 종료됩니다.\n미 결제시 수업이 종료될 수 있습니다. ";
		} else if($SendMessagePerDay==7) {
			$MsgDate =  date('Y-m-d', strtotime($NowDate. ' +7 day'));
			$SendMessage="$StrDomain $MemberName 님의 수업이 $MsgDate 일자로 종료됩니다.\n수강료 결제 부탁드립니다. ";
		}
		$SendMemo="";
		...
		$StrSmsMessagePhoneNumber = $SmsMessagePhoneNumber."/".$SendMessagePerDay; // 1,3,7 로 바뀌면서 넘버와 일자도 비교
		if(preg_match("/^01[0-9]{8,9}$/", $MemberPhone1Check) && $UseSendSms == 1) { // 발송 대상이면서 유효성검사에 통과한 값들만
			if( strpos($CheckReSend, $StrSmsMessagePhoneNumber)!==false ) { // 보낸폰번호 이력에 해당 번호가 있다면 ( 기존에 보냈었다면 )
				$SmsCheckResult = 0;
			} else { // 보낸폰번호 이력에 없다면... 
				$CheckReSend = $CheckReSend . $StrSmsMessagePhoneNumber . "|";
				$SmsCheckResult = 1;
			}
		} else {
			$SmsCheckResult = 0;
		...
		$StrSmsMessagePhoneNumber2 = $SmsMessagePhoneNumber2."/".$SendMessagePerDay; // 1,3,7 로 바뀌면서 넘버와 일자도 비교
		if(preg_match("/^01[0-9]{8,9}$/", $MemberPhone2Check) && $UseSendSms == 1) { // 발송 대상이면서 유효성검사에 통과한 값들만
			if( strpos($CheckReSend, $StrSmsMessagePhoneNumber2)!==false ) { // 보낸폰번호 이력에 해당 번호가 있다면 ( 기존에 보냈었다면 )
				$SmsCheckResult = 0;
			} else { // 보낸폰번호 이력에 없다면... 
				$CheckReSend = $CheckReSend . $StrSmsMessagePhoneNumber2 . "|";
				$SmsCheckResult = 1;
			}
		} else {
			$SmsCheckResult = 0;

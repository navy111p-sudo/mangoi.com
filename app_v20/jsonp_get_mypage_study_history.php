...
while($Row = $Stmt->fetch()) {
	...
	if ($AssmtStudentDailyScoreID){
		...
		$PageMyPageHistoryHTML .= "		<div class=\"my_study_start\">레스비디오 : <b>".$ClassVideoPlayCount."회 시청</b></div>";
		$PageMyPageHistoryHTML .= "		<div class=\"my_study_finish\">리뷰퀴즈 : <b>".$ClassQuizCount."회 완료</b></div>";
		$PageMyPageHistoryHTML .= "	</div>";
		$PageMyPageHistoryHTML .= "	<div class=\"my_study_btns\">";
		$PageMyPageHistoryHTML .= "		<div class=\"my_study_incorrect\">";
		$PageMyPageHistoryHTML .= "			<h5 class=\"my_study_incorrect_caption\">퀴즈결과</h5>";
		...
		while($Row2 = $Stmt2->fetch()) {
			...
			if ($BookQuizResultState==1){
				...
				$PageMyPageHistoryHTML .= "				<li><span class=\"incorrect_btn_on\" onclick=\"OpenStudyQuiz2(".$BookQuizID.",".$ClassID.",".$QuizStudyNumber.", ".$BookRegForReason.")\">풀기</span></li>";
			
			}else{

				$PageMyPageHistoryHTML .= "				<li><span class=\"incorrect_btn_off\" onclick=\"OpenStudyQuizResult(".$BookQuizResultID.")\">".$ii2."회</span></li>";
		...
		if ($BookQuizID!=0){
			if ($BookQuizResultState==2){
				$PageMyPageHistoryHTML .= "				<li><span class=\"incorrect_btn_on\" onclick=\"OpenStudyQuiz2(".$BookQuizID.",".$ClassID.",".($QuizStudyNumber+1).", ".$BookRegForReason.")\">재응시</span></li>";
			}
		}

		$PageMyPageHistoryHTML .= "			</ul>";
		$PageMyPageHistoryHTML .= "		</div>";
		$PageMyPageHistoryHTML .= "		<a href=\"".$StrDailyReportLink."\" class=\"my_study_download\" style=\"".$StrDailyReportStyle."\"><img src=\"images/icon_bbs_3.png\" class=\"icon\">리포트</a>";
		$PageMyPageHistoryHTML .= "	</div>";
		$PageMyPageHistoryHTML .= "</li>";

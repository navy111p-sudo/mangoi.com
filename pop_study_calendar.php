...
<div class="mantoman_write_wrap">
	<div class="mantoman_write_area" style="width:90%;">
        <?php
        /*  학생 레벨:  일반 학생은 16 이상, 학생이 아닌 계정(교사·센터·관리자 등)은 15 이하
            → 학생(LEVEL > 15)이 아니면 표를 보여준다
        */
        if (isset($_LINK_MEMBER_LEVEL_ID_) && $_LINK_MEMBER_LEVEL_ID_ <= 15) {
            ?>
            <table style="float:left;border:1px solid #CCCCCC;border-collapse:collapse;margin:0 10px 10px 0;font-size:20px;">
                <tr>
                    <th style="background:#F1F1F1;padding:5px;">학생명</th>
                    <td style="padding:5px;"><?php echo htmlspecialchars($Row["MemberName"]); ?></td>
                </tr>
                <tr>
                    <th style="background:#F1F1F1;padding:5px;">아이디</th>
                    <td style="padding:5px;"><?php echo htmlspecialchars($Row["MemberLoginID"]); ?></td>
                </tr>
            </table>
        <?php } ?>

        <h3 class="caption_underline" style="<?php echo (strpos($_SERVER['REQUEST_URI'], '/lms/') !== false) ? 'margin-left:140px;' : ''; ?>">
			<trn class="TrnTag">이달의 학습일정</trn>
			<div  style="margin-top:20px;">
				<a href="?SelectYear=<?=$p_year?>&SelectMonth=<?=$p_month?>&MemberID=<?=$MemberID?>&IframeMode=<?=$IframeMode?>" class="calendar_arrow left" style="color:#999999;">◀</a>
				<?=$year?>.<?=substr("0".$month,-2)?>
				<a href="?SelectYear=<?=$n_year?>&SelectMonth=<?=$n_month?>&MemberID=<?=$MemberID?>&IframeMode=<?=$IframeMode?>" class="calendar_arrow right" style="color:#999999;">▶</a>
			</div>

		</h3> 
		<div style="text-align:right;margin-bottom:10px;padding-right:10px;">※ 종료일자는 정규 종료일자 입니다. 연기/보강/변경 등은 종료일자 이후라도 수업이 진행됩니다.</div>
		<table class="level_reserve_table"  align="center">
			...
			<tr>
				...
				?> 
					...
					if ($today-$NowDate==0 || date('w', $NowDate)==0 || date('w', $NowDate) == 6){
					...
					?>
						...
						$Sql2_ViewTable = "
							select 
								AAAA.* 
							from ClassOrderSlots AAAA 
								inner join ClassOrders BBBB on AAAA.ClassOrderID=BBBB.ClassOrderID 
							where 
									1=1 
									and AAAA.ClassOrderSlotMaster=1 
									and ( 
											(AAAA.ClassOrderSlotType=1 and AAAA.StudyTimeWeek=".$SelectWeek." and AAAA.ClassOrderSlotStartDate is NULL and AAAA.ClassOrderSlotEndDate is NULL ) 

											or 
											(AAAA.ClassOrderSlotType=1 and AAAA.StudyTimeWeek=".$SelectWeek." and datediff(AAAA.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and AAAA.ClassOrderSlotEndDate is NULL ) 

											or 
											(AAAA.ClassOrderSlotType=1 and AAAA.StudyTimeWeek=".$SelectWeek." and AAAA.ClassOrderSlotStartDate is NULL and datediff(AAAA.ClassOrderSlotEndDate, '".$SelectDate."')>=0 ) 

											or 
											(AAAA.ClassOrderSlotType=1 and AAAA.StudyTimeWeek=".$SelectWeek." and datediff(AAAA.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and datediff(AAAA.ClassOrderSlotEndDate, '".$SelectDate."')>=0 ) 

											or 
											(AAAA.ClassOrderSlotType=2 and datediff(AAAA.ClassOrderSlotDate, '".$SelectDate."')=0 )   
										)  
									and AAAA.ClassOrderSlotState=1 

									and BBBB.MemberID=".$MemberID." 
									and (BBBB.ClassOrderState=1 or BBBB.ClassOrderState=2 or BBBB.ClassOrderState=3 or BBBB.ClassOrderState=5 or BBBB.ClassOrderState=6)

							group by AAAA.TeacherID, AAAA.ClassOrderID, AAAA.StudyTimeWeek, AAAA.StudyTimeHour, AAAA.StudyTimeMinute, AAAA.ClassOrderSlotState
						";
                      ...
                      from ($Sql2_ViewTable) COS 

                            inner join ClassOrders CO on COS.ClassOrderID = CO.ClassOrderID

                                    left outer join Classes CLS
                                       on CLS.ClassOrderID = COS.ClassOrderID
                                      and CLS.MemberID    = CO.MemberID
                                      and CLS.StartYear   = ".$year."
                                      and CLS.StartMonth  = ".$month."
                                      and CLS.StartDay    = ".$day."
                                      and CLS.StartHour   = COS.StudyTimeHour
                                      and CLS.StartMinute = COS.StudyTimeMinute
                                      and CLS.TeacherID   = COS.TeacherID
                                      and CLS.ClassAttendState <> 99
                      ...
                      where 
                            ...
                            /* ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ 여기를 수정합니다 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ */
                            and (CO.ClassOrderStartDate IS NULL OR '".$SelectDate."' >= CO.ClassOrderStartDate) /* 이 라인과 */
                            and (
                                  CO.ClassOrderEndDate IS NULL
                                  or '".$SelectDate."' <= CO.ClassOrderEndDate
                                  or (CT.CenterPayType=1 and MB.MemberPayType=0 and (CO.ClassOrderState=1 or CO.ClassOrderState=2 or CO.ClassOrderState=5 or CO.ClassOrderState=6))
                            )   /* B2B(전체결제)는 종료일이 지나도 노출, 단 수업종료(상태=3)면 종료일 이후 미노출 */
                            and COS.ClassOrderSlotMaster=1 
                            and ( 
                                  (COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectWeek." and COS.ClassOrderSlotStartDate is NULL and COS.ClassOrderSlotEndDate is NULL ) 

                                  or 
                                  (COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectWeek." and datediff(COS.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and COS.ClassOrderSlotEndDate is NULL ) 

                                  or 
                                  (COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectWeek." and COS.ClassOrderSlotStartDate is NULL and datediff(COS.ClassOrderSlotEndDate, '".$SelectDate."')>=0  ) 

                                  or 
                                  (COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectWeek." and datediff(COS.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and datediff(COS.ClassOrderSlotEndDate, '".$SelectDate."')>=0 ) 
                   ...
                   ";
						...
						while($Row2 = $Stmt2->fetch()) {
							...
							$ClassOrderSlotID = $Row2["ClassOrderSlotID"];
                            ...
                            /* Classes 가 붙지 않았거나 – 다른 학생이면 skip */
                            if ($Row2['MemberID'] != $MemberID) {
                                continue;
                            }
						...
						?>
							...
							<div style="line-height:1.5;margin-bottom:20px;border:1px solid #CACACA;padding:10px;border-radius:5px;">
								...
								<?}else if ($ClassProductID==2 && $AssmtStudentLeveltestScoreID!=0){?>
									<br>
									<span style="display:inline-block;width:50px;text-align:center;font-size:10px;margin-top:10px;padding:5px;border-radius:3px;background-color:#888888;color:#ffffff;cursor:pointer;" onclick="javascript:OpenStudentScoreLeveltestReport(<?=$AssmtStudentLeveltestScoreID?>);">REPORT</span>
								<?}?>
								
								<?if ($ClassAttendState<=8){?>
...
function OpenClassListMini(SelectDate, TeacherID, StudyTimeWeek, StudyTimeHour, StudyTimeMinute){
	var openurl = "./lms/class_list_mini.php?SelectDate="+SelectDate+"&TeacherID="+TeacherID+"&StudyTimeWeek="+StudyTimeWeek+"&StudyTimeHour="+StudyTimeHour+"&StudyTimeMinute="+StudyTimeMinute+"&IframeMode=<?=$IframeMode?>";
	if(<?=$IframeMode?>==1) {
		window.open(openurl, "class_list_mini", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,left=50,width=1080,height=500");
	} else {
		$.colorbox({	
			href:openurl
			,width:"95%" 
...
function OpenTeacherMessageForm(TeacherID){
    openurl = "./lms/teacher_message_form.php?TeacherID="+TeacherID+"&IframeMode=<?=$IframeMode?>";
	if(<?=$IframeMode?>==1) {
		window.open(openurl, "class_schedule_by_teacher", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,left=50,width=1080,height=800");
	} else {
		$.colorbox({    
...
function OpenStudentScoreDailyReport(ClassID){
	...
	var openurl = "./report_daily.php?ClassID="+ClassID+"&IframeMode=<?=$IframeMode?>";
	if(<?=$IframeMode?>==1) {
		window.open(openurl, "student_score_daily_report", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,left=50,width=1080,height=800");
	} else {
		$.colorbox({	
			href:openurl
			,width:"95%" 
...
function OpenStudentScoreLeveltestReport(AssmtStudentLeveltestScoreID){
	var openurl = "./report_leveltest.php?AssmtStudentLeveltestScoreID="+AssmtStudentLeveltestScoreID+"&IframeMode=<?=$IframeMode?>";
	if(<?=$IframeMode?>==1) {
		window.open(openurl, "student_score_leveltest_report", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,left=50,width=1080,height=800");
	} else {
		$.colorbox({	
			href:openurl
			,width:"95%" 

    ...
    <section class="mypage_wrap">
        <div class="mypage_area">
            ...
            <ul class="class_register_tab">
                <li><a href="class_register_teacher.php" class="active TrnTag"><span class="register_img_1"></span>강사 먼저 선택</a></li>
                <li><a href="class_register_time.php" class="TrnTag"><span class="register_img_2"></span>날짜 먼저 선택</a></li>
            </ul>
            ...
            <div class="mypage_inner">
                <h3 class="caption_left_common TrnTag"><b>키워드</b> 검색<span>* 아래 키워드를 선택하시면 해당 키워드의 강사들을 찾으 실 수 있습니다. (중복선택가능)</span></h3>
				...
				<ul class="class_keyword_list noborder">
					<li class="class_keyword_caption nation">강사 구분</li>
				...
				<ul class="class_keyword_list">
					<li class="class_keyword_caption time TrnTag">수강 시간</li>
                ...
                <ul class="class_keyword_list week noborder">
                    <li class="class_keyword_caption time TrnTag">수업 회수</li>
				...
				function ChChar(TeacherCharacterItemID){
					...
					if (SearchLoad==0){
						alert( '3개까지 선택할 수 있습니다. 이미 선택한 항목을 해제해 주세요.');
					}else{
				...
				function GetTeacherList(SetType, SearchTeacherID){
					if (SetType==0){
						...
						if (GetTimeTeachers.indexOf("|"+SearchTeacherID+"|") == -1) {
							...
							//window.open(url + "?EduCenterID="+EduCenterID+"&SearchSex="+SearchSex+"&SearchChar="+SearchChar+"&SearchTeacherPayTypeItemID="+SearchTeacherPayTypeItemID+"&ClassOrderStartDate="+ClassOrderStartDate, 'ajax_get_class_order_teacher_list');
...
$('.teacher_time_table span').each(function(index, element) {
  // span 요소의 onclick 속성에서 함수 인자 추출
  var onclickAttr = $(element).attr('onclick');
  
  if (onclickAttr) {
    // 정규식으로 SelectSlot 함수의 인자를 추출
    var args = onclickAttr.match(/\(([^)]+)\)/)[1].split(',');

    // 추출한 값을 출력
    console.log('Arguments: ', args); // ['24', '1', '32', '1']
...
var result = SelectSlotTest(Number(arg1), Number(arg2), Number(arg3), Number(arg4));	
				...
				function SelectSlot(TeacherID, WeekDayNum, MinuteListNum, TeacherBlock80Min){//TeacherBlock80Min - 1:80분 제한 / 0:제한안함
					...
					if ($("#Able_"+TeacherID+"_"+WeekDayNum+"_"+MinuteListNum).val()=="1"){
						...
						if (SelectSlotCode.indexOf(SlotCode)<0 && SelectedSlotCount>=(ClassOrderWeekCount*ClassOrderTimeSlotCount)){
							alert( "주" + ClassOrderWeekCount + '회까지 선택할 수 있습니다.');
						}else if (SelectStudyTimeCode.indexOf(CheckStudyTimeCode)>=0 && SelectStudyTimeCode.indexOf(StudyTimeCode)<0){
							alert( "동일한 요일에 동일한 강사를 한번만 선택할 수 있습니다.");
						}else{
							if (DenySelect==1){
								alert( (ClassOrderTimeSlotCount*10) + '분 수업을 구성할 수 없습니다.');
							}else if (DenySelect==2){
								alert( '선택한 시간은 휴식시간 입니다.');//80분 이상 연속수업 금지 정책에 위배됩니다.
							}else{
...
function SelectSlotTest(TeacherID, WeekDayNum, MinuteListNum, TeacherBlock80Min){
					...
					if ($("#Able_"+TeacherID+"_"+WeekDayNum+"_"+MinuteListNum).val()=="1"){
						...
						if (SelectSlotCode.indexOf(SlotCode)<0 && SelectedSlotCount>=(ClassOrderWeekCount*ClassOrderTimeSlotCount)){
							...
							if (DenySelect==1){
								...
								return {
									type: 1,
									message: (ClassOrderTimeSlotCount*10) + '분 수업을 구성할 수 없습니다.'
								};
							}else if (DenySelect==2){
								//alert( '선택한 시간은 휴식시간 입니다.');//80분 이상 연속수업 금지 정책에 위배됩니다.
return {
									type: 2,
									message: '선택한 시간은 휴식시간 입니다.'
								};
							}else{
AbleNum = 0;
								return {
...
function ClassOrderSubmit(){
    if (SelectedSlotCount < (ClassOrderWeekCount*ClassOrderTimeSlotCount)){
        alert( "주" + ClassOrderWeekCount + '회 선택하셔야 합니다.');
        ...
        if (confirm("선택한 조건으로 수강신청을 진행하시겠습니까?")){
            ...
            $.ajax(url, {
                ...
                success: function (data) {
                    ...
                    // 여기서 모바일 확인 로직을 추가합니다.
                    if (isMobileDevice()) {
                        // PayPreActionMobile(ClassOrderID);
...
function PayPreActionMobile(ClassOrderID){
    url = "./ajax_set_class_order_pay.php";
    $.ajax(url, {
        data: {
            ClassOrderID: ClassOrderID,
        ...
        success: function (data) {
            ClassOrderPayID = data.ClassOrderPayID;
            ClassOrderPayNumber = data.ClassOrderPayNumber;
            var openurl = "./pop_class_order_pay_form.php?ClassOrderID="+ClassOrderID+"&ClassOrderPayID="+ClassOrderPayID+"&ClassOrderPayNumber="+ClassOrderPayNumber+"&ClassOrderMode=HOME";

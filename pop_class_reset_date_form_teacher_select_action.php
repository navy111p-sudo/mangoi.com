...
$StudyTimeDate = isset($_REQUEST["StudyTimeDate"]) ? $_REQUEST["StudyTimeDate"] : "";
if(empty($StudyTimeDate)) {
    die("StudyTimeDate 값이 전달되지 않았습니다.");
}


// --------------------------------------------------------------------------------------------
// DataCredit 수정 - 2024-02-05 :: 스케줄 변경은 수업 시간 24시간 전에만 가능하도록 수정
...
//
//// 스케줄 변경 날짜 및 시간 설정
//$changeDateTime = new DateTime($StudyTimeDate . ' ' . $StudyTimeHour . ':' . $StudyTimeMinute);
//
//// 현재 날짜 및 시간
//$currentDateTime = new DateTime();
//
//// 스케줄 변경이 현재 시간으로부터 24시간 이전인지 확인
//$interval = $currentDateTime->diff($changeDateTime);
//$hours = $interval->h;
//$hours = $hours + ($interval->days*24);
//
//// 24시간 이내에는 스케줄 변경 불가
//if ($hours < 24) {
//    echo "<script>alert('스케줄 변경은 수업 시간 24시간 전에만 가능합니다.'); history.go(-1);</script>";
//    exit;
//}
...
// 2025-03-25 :: DataCredit 작업 - 장지웅 부장님 요청: 토, 일요일에 대해서도 가능하도록 수정 필요
// 토, 일요일 검사 코드 제거함.
	...
	for ($ii=1;$ii<=count($ArrClassID)-2;$ii++){
		...
		for ($iiii=1;$iiii<=$ClassOrderTimeTypeID;$iiii++){
			...
			$Sql = " insert into ClassOrderSlots ( ";
				...
				if($ClassAttendState==8 or $ClassAttendState==4 or $ClassAttendState==5) {
					$Sql .= " ClassOrderPayID, ";
				}
				$Sql .= " ClassMemberType, ";

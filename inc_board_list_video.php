...
?>
	...
	function showThumb(data){
		...
		//$(id_img).attr('width',"237");
		//$(id_img).attr('height',"165");
	}
	...
	<div id="bbs">
		...
		<ul class="TabGallery">
			...
			?>
			<li><a href="board_list.php?BoardCode=<?=$BoardCode?>&PageListNum=<?=$PageListNum?>&BoardCategoryID=<?=$Row2["BoardCategoryID"]?>" <?if ($BoardCategoryID==$Row2["BoardCategoryID"]){?>class="Active"<?}?>><?=$Row2["BoardCategoryName"]?></a></li>
			<?
			}
			$Stmt2 = null;
			?>
		...
		<ol class="Gallery">
			...
			?>
			<li>
				<?if ($BoardContentVideoCode!=""){?>
					<a href="<?=$LinkUrl?>"><img src="" id="vimeo-<?=$BoardContentVideoCode?>"></a>
					<script>vimeoLoadingThumb('<?=$BoardContentVideoCode?>')</script>
				<?}else{?>
					<a href="<?=$LinkUrl?>"><img src="images/no_image.png"></a>
				<?}?>

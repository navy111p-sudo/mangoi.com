...
if($ClassID) {
	$Sql2 = "
		...
		where 
			A.ClassID=$ClassID and A.BookQuizResultState=2 
...
// /*
// 결과 가 있다면 
if($BookQuizResultID>0) {
		header("Location: ./pop_quiz_study_result.php?BookQuizResultID=$BookQuizResultID&FromDevice=$FromDevice");
} else {
	if($ClassID) {
		echo "<script>window.Exit=true;alert('완료하신 퀴즈가 없습니다.'); </script>";

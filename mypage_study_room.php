    ...
    <section class="mypage_wrap">
        <div class="mypage_area" >
            ...
            <div class="schedule_wrap" style="margin-top:30px;">
				...
				$Stmt5->bindParam(':MemberID', $_LINK_MEMBER_ID_);
				$Stmt5->execute();
				...
				<div class="schedule_btns_right">
                    ...
                    <a href="product_order_cart.php" class="button_orange_white lesson" style="background-color:#808040;">교재주문하기 (<?=$ProductOrderCartCount?>)</a>
					<?}else{?>
					<a href="javascript:ProductOrderCartErr()" class="button_orange_white lesson" style="background-color:#c1c1c1;">교재주문하기 (0)</a>
					<?}?>
					<a href="product_order_list.php" class="button_whtie_border_arrow schedule">교재주문내역</a>
                </div>
				<script>
				function ProductOrderCartErr(){
					alert("현재 주문할 교재가 없습니다.\n\n주문할 교재는 선생님께서 선택해 주십니다.");
				}
				</script>
				<?
				//================== 교재 구매 버튼 ====================
				...
				<ul class="schedule_list">
					...
					if ($_LINK_MEMBER_LOGIN_ID_=="test01" || $_LINK_MEMBER_LOGIN_ID_=="test02" || $_LINK_MEMBER_LOGIN_ID_=="test03"){ 
							//tes01의 수업
							...
							//test02의 수업
							if ($_LINK_MEMBER_LOGIN_ID_=="test02"){ 
							...
							//test03의 수업
							if ($_LINK_MEMBER_LOGIN_ID_=="test03"){ 
					...
					?>
						...
						<li style='border:2px solid #FDC644'>
							...
							<ul class="schedule_btns">
								...
								<li style="width:100%;"><a href="javascript:OpenClassShPreSet('322029', '151_20230704_14_0', 0, '테스트', '<?=$_LINK_MEMBER_LOGIN_ID_?>');"  class="color_classin">수업입장</a></li>

								<li><a href="javascript:OpenStudyVideo_Sample(1, '<?=$LessonAVideo?>', -1, 1);" class="color_lesson_video">레슨비디오 A</a></li> 
								<li><a href="javascript:OpenStudyVideo_Sample(1, '<?=$LessonBVideo?>', -1, 1);" class="color_lesson_video">레슨비디오 B</a></li> 
								<li><a href="javascript:OpenBookScan_Sample('<?=$LessonBook?>',-1);" class="color_review_book">학습교재</a></li>
								<li><a href="javascript:OpenStudyQuiz_Sample(<?=$ReviewQuiz?>, -1);" class="color_review_quiz">리뷰퀴즈</a></li>


								<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateForm_Sample(-1, 1);" class="color_postpone">연기 및 스케줄 변경</a></li>
					...
					for ($iiii=$SelectYearMonthStartDay;$iiii<=$SelectYearMonthEndDay;$iiii++){
						...
						if ($TotalRowCount>0){
							...
							while($Row = $Stmt->fetch()) {
								...
								// 4회 이상 수업을 진행했는지 확인하고 진행했으면 리뷰쓰기 권한을 준다.
								$Sql3 = "SELECT COUNT(*) as class_count FROM Classes  
										WHERE TeacherID = :TeacherID AND MemberID = :MemberID AND ClassState = 2 ";
									...
									<ul class="schedule_btns">
										...
										<?}else{?>
											...
											?>
												...
												<li style="width:100%;"><a href="javascript:<?=$reviewWriteMsg?>" class="color_classin TrnTag" style="background-color:#ff7f00;">강사리뷰작성</a></li>
													...
													<?if ($ClassID==0){?>
														...
														<?if ($ClassMemberType==1){// 1:1 수업?>	
															<?if($MemberID==13591){// anlab97 학생 말아달라고 요청 by 강주임 200107?>
																<li style="width:100%;" id="BtnResetDate"><a class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
															<?}else{?>
																<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateFormWithReg(<?=$ClassOrderID?>, <?=$SelectYear?>, <?=(int)$SelectMonth?>, <?=(int)$SelectDay?>, <?=(int)$StudyTimeWeek?>,<?=(int)$StudyTimeHour?>, <?=(int)$StudyTimeMinute?>,	<?=$ClassOrderTimeTypeID?>, <?=$TeacherID?>, <?=$MemberID?>, <?=$ClassMemberType?>, <?=$ClassProductID?>);" class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
															<?}?>
														<?}else{?>
															<?if($MemberID==13591){// anlab97 학생 말아달라고 요청 by 강주임 200107?>
																<li style="width:100%;" id="BtnResetDate"><a  class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
															<?}else{?>
																<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateFormErr();" class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
															<?}?>
														<?}?>
														...
														<?}else{?>
															<?if ($ClassMemberType==1){// 1:1 수업?>	
																<?if($MemberID==13591){// anlab97 학생 말아달라고 요청 by 강주임 200107?>
																	<li style="width:100%;" id="BtnResetDate"><a class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
																<?}else{?>
																	<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateForm(<?=$ClassID?>, <?=$ClassProductID?>, <?=(int)$StudyTimeWeek?>, <?=(int)$StudyTimeHour?>, <?=(int)$StudyTimeMinute?>);" class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
																<?}?>
															<?}else{?>
																<?if($MemberID==13591){// anlab97 학생 말아달라고 요청 by 강주임 200107?>
																	<li style="width:100%;" id="BtnResetDate"><a class="color_postpone TrnTag">연기 및 스케줄 변경</a></li>
                ...
                <table class="mypage_survey_wrap">
                    <tr>
                        ...
                        <td class="survey_left">
                            ...
                            <trn class="TrnTag">궁금하거나 부족한점 있으면 의견을 남겨주세요.<br>항상 최선을 다하겠습니다.</trn>
                        </td>
...
function OpenResetDateFormWithReg(ClassOrderID, SelectYear, SelectMonth, SelectDay, StudyTimeWeek, StudyTimeHour, StudyTimeMinute, ClassOrderTimeTypeID, TeacherID, MemberID, ClassMemberType, ClassProductID){
    ...
    $.ajax(url,{    
		...
		success: function(data) {
			...
			OpenResetDateForm(data.ClassID, ClassProductID, StudyTimeWeek, StudyTimeHour, StudyTimeMinute);
...
function OpenResetDateForm(ClassID, ClassProductID, SetWeek, SetHour, SetMinute){
    ...
    $.ajax(url,{    
		...
		success: function(data) {
			...
			if (EnableClassTimeReset==1){
				var OpenUrl = "pop_class_reset_date_form.php?ClassID="+ClassID+"&ClassProductID="+ClassProductID+"&SetWeek="+SetWeek+"&SetHour="+SetHour+"&SetMinute="+SetMinute;
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.AssmtStudentSelfScoreID==0){
				...
				if(OnlineSiteShVersion==1) {
					var CommonShNewClassCode = data.CommonShNewClassCode;
					OpenClassSh(ClassID, CommonShNewClassCode, MemberType, MemberName, MemberLoginID, OnlineSiteShVersion, CommonShClassCode);
				} else if(OnlineSiteShVersion==2) {
					OpenClassSh(ClassID, CommonShClassCode, MemberType, MemberName, MemberLoginID, OnlineSiteShVersion, CommonShClassCode);
				}
			}
...
function OpenClassSh(ClassID, CommonShClassCode, MemberType, MemberName, MemberLoginID, OnlineSiteShVersion, originCommonShCode){
	...
	if(OnlineSiteShVersion==1) {
		...
		MvApi.defaultSettings({
			...
			client: {
				...
				mac: {
					...
					store: true, 
					// 스킴 이름
					scheme: 'cloudboda', //사설 배포 : mangoi
					// 패키지 이름
					packagename: 'zone.cloudboda', //사설 배포 : zone.mangoi
				},
				// 사용언어 - 없으면 한국어
				language: '<?=$ShLanguage?>',
		...
		console.log(CommonShClassCode);
		$('input[name=userId]').val(MemberLoginID);
		//$('input[name=title]').val("망고아이 수업");
		$('input[name=title]').val("망고아이"+originCommonShCode);
...
var OpenUrl = "review_write_form.php?TeacherID="+TeacherID+"&isRedirect=0";
$.colorbox({	
	href:OpenUrl
	,width:"95%" 
	,height:"95%"

...
<div class="sub_wrap">       
    <div class="sub_title_common_wrap" ><h2 class="sub_title_common"><b>나의</b> 공부방 </h2></div>
        ...
        <div class="mypage_area" >
            ...
            <div class="schedule_wrap" style="margin-top:30px;">
				...
				<div class="schedule_btns_right">
                    ...
                    <a href="javascript:ProductOrderCart(<?=$_LINK_MEMBER_ID_?>)" class="button_orange_white lesson" style="background-color:#808040;">교재주문하기 (<?=$ProductOrderCartCount?>)</a>
					<?}else{?>
					<a href="javascript:ProductOrderCartErr()" class="button_orange_white lesson" style="background-color:#c1c1c1;">교재주문하기 (0)</a>
					<?}?>
					<a href="javascript:ProductOrderList(<?=$_LINK_MEMBER_ID_?>)" class="button_whtie_border_arrow schedule">교재주문내역</a>
                </div>
				<script>
				function ProductOrderCart(uid){
					//var OpenUrl = "../product_order_cart.php?FromDevice=mypage";
					var OpenUrl = "../product_order_login_from_mypage.php?uid="+uid+"&page=1";
					$.colorbox({    
						href:OpenUrl
						,width:"95%" 
						,height:"95%"
				...
				function ProductOrderList(){
					//var OpenUrl = "../product_order_list.php?FromDevice=mypage";
					var OpenUrl = "../product_order_login_from_mypage.php?uid="+uid+"&page=2";
					$.colorbox({    
						href:OpenUrl
						,width:"95%" 
						,height:"95%"
				...
				?>
				<div style="width:100%;height:30px;text-align:center;background-color:#cccccc;color:#ffffff;border-radius:10px;margin-top:20px;margin-bottom:20px;line-height:30px;cursor:pointer;" onclick="location.href='mypage_study_room.php?GetAllList=1'">지난수업보기</div>
				<?
					}
				}
				?>
								...
								<li <?=$LiStyle?>>
									...
									<ul class="schedule_btns">
										...
										<?if ($TodayIsHoliday==1 && $ClassState!=2){?>
											<li style="width:100%;">
												<a class="color_classin" style="background-color:#888888;">휴무일 - 연기처리(대상)</a>
											</li>
										<?}else{?>
											...
											?>
												...
												<?if ($ClassState==0){//등록완료 0:미등록 전 1:등록완료 2:수업완료?>
													<?if ($CurrentDateType==0 || $CurrentDateType>0){?>
														<li style="width:100%;"><a class="color_classin" style="background-color:#F3A39E;">수업준비중</a></li>
													<?}else{?>
														<li style="width:100%;"><a class="color_classin" style="background-color:#888888;">경과된수업</a></li>
													<?}?>
												<?}else{//등록완료 0:미등록 전 1:등록완료 2:수업완료?>
													
													<?if ($ClassState==2){//등록완료 0:미등록 전 1:등록완료 2:수업완료?>
															<li style="width:100%;"><a class="color_classin" style="background-color:#888888;">종료된수업</a></li>
													<?}else{?>
														<?if ($CurrentDateType==0 || $CurrentDateType>0){?>
															<?if ($ClassLinkType==1){?>
																<li style="width:100%;"><a href="javascript:OpenClassShPreSet('<?=$ClassID?>', '<?=$CommonShClassCode?>', 0, '<?=$MemberName?>', '<?=$MemberLoginID?>');" class="color_classin">수업입장</a></li>
															<?}else{?>
																<li style="width:100%;"><a href="javascript:OpenClassCiCheckPreSet('<?=$ClassID?>', '<?=$CommonCiTelephoneTeacher?>', '<?=$CommonCiTelephoneStudent?>', 1, '<?=$MemberName?>', 'MangoiClass_<?=$ClassID?>');" class="color_classin">수업입장</a></li>
															<?}?>
														<?}else{?>
															<li style="width:100%;"><a class="color_classin" style="background-color:#888888;">경과된수업</a></li>
														<?}?>
													...
													<?if ($ClassProductID!=2){?>
														<li><a href="javascript:<?=$LinkVideo?>;" class="color_lesson_video">레슨비디오 A</a></li> 
														<li><a href="javascript:<?=$LinkVideo2?>;" class="color_lesson_video">레슨비디오 B</a></li> 
														<li><a href="javascript:<?=$LinkScan?>;" class="color_review_book">학습교재</a></li>
														<li><a href="javascript:<?=$LinkQuiz?>;" class="color_review_quiz">리뷰퀴즈</a></li>
													<?}?>
												...
												<?if ($CurrentDateType==0 || $CurrentDateType>0){?>
													...
													<?if ($ClassID==0){?>
														...
														<?if ($ClassMemberType==1){// 1:1 수업?>	
															<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateFormWithReg(<?=$ClassOrderID?>, <?=$SelectYear?>, <?=(int)$SelectMonth?>, <?=(int)$SelectDay?>, <?=(int)$StudyTimeHour?>, <?=(int)$StudyTimeMinute?>, <?=$ClassOrderTimeTypeID?>, <?=$TeacherID?>, <?=$MemberID?>, <?=$ClassMemberType?>, <?=$ClassProductID?>);" class="color_postpone">수업연기요청</a></li>
														<?}else{?>
															<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateFormErr();" class="color_postpone">수업연기요청</a></li>
														<?}?>
													...
													<?}else{?>
														...
														<?}else{?>
															<?if ($ClassMemberType==1){// 1:1 수업?>	
																<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateForm(<?=$ClassID?>, <?=$ClassProductID?>, <?=(int)$StudyTimeHour?>, <?=(int)$StudyTimeMinute?>);" class="color_postpone">수업연기요청</a></li>
															<?}else{?>
																<li style="width:100%;" id="BtnResetDate"><a href="javascript:OpenResetDateFormErr();" class="color_postpone">수업연기요청</a></li>
															<?}?>
														<?}?>
												...
												<?if ($ClassProductID!=2 && $AssmtStudentDailyScoreID!=0){//데일리 평가보고?>
													<li style="width:100%;"><a href="javascript:OpenStudentScoreDailyReport(<?=$ClassID?>);" class="color_review_quiz" style="background-color:#3D3D3D;">평가보고서</a></li>
												<?}else if ($ClassProductID==2 && $AssmtStudentLeveltestScoreID!=0){?>
													<li style="width:100%;"><a href="javascript:OpenStudentScoreLeveltestReport(<?=$AssmtStudentLeveltestScoreID?>);" class="color_review_quiz" style="background-color:#3D3D3D;">평가보고서</a></li>
												<?}?>

												<?if ($ClassState==2 && $ClassProductID==1){?>
													<li style="width:100%;"><a href="javascript:OpenTeacherScoreForm(<?=$ClassID?>);" class="color_review_quiz" style="background-color:#0080C0;">강사수업평가</a></li>
												<?}?>
		...
		success: function(data) {
			...
			if (EnableClassTimeReset==1){
				...
				$.colorbox({    
				 ...
				 alert("수업시간 연기는 수업시작 2시간 전까지 가능합니다.");

...
if ($DomainSiteID == 9 && $BoardCode == "notice") {
    $TotalRowCount = 0;
    $TotalPageCount = 0;
    $Stmt = null;
} else {
...
?>
	...
	<div id="bbs">
		<div class="TopSearch">
        	<span>Total : <b><?=$TotalRowCount?></b></span>
			...
			<input type="hidden" name="PageListNum" value="<?=$PageListNum?>">
					...
					<select name="SearchItem" class="Select">
						<option value="1" <?php if ($SearchItem=="1") {echo "selected";}?> class="TrnTag">제목+내용</option>
						<option value="2" <?php if ($SearchItem=="2") {echo "selected";}?> class="TrnTag">제목</option>
						<option value="3" <?php if ($SearchItem=="3") {echo "selected";}?> class="TrnTag">내용</option>
						<option value="4" <?php if ($SearchItem=="4") {echo "selected";}?> class="TrnTag">작성자</option>
					</select>
					<input type="text" name="SearchText" class="Input2" placeholder="검색어를 입력해 주세요." value="<?=$SearchText?>"><a href="javascript:SearchSubmit()" class="BtnSearch TrnTag">검색</a>
        ...
        <div class="BtnRight"><a href="board_form.php?BoardCode=<?=$BoardCode?>" class="BtnGray TrnTag">글쓰기</a></div>
		<?php
		}else{
		?>

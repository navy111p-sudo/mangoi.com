...
// 2024-04-08 모바일 결제 결과
// res_msg: 정상처리
//escw_used: N
//req_tx: pay
//card_point_use: N
//trace_no: T0000482SISKKGBH
//buyr_tel2: 01072922400
//buyr_mail: datacredit2021@gmail.com
//buyr_tel1: 01072922400
//tran_cd: 00100000
//param_opt_1: rdvDN15
//enc_info: 2ny7VxDacJ61l1CisA.5HJ2NOSchaaefipUcvabvj8yYtMctW9fPs6S79CfhKnqIQCp.F4T7-WU4v8qYusFfAIu0LIPtTyViZqES3m-dD5uw8wNI-tz-McsZDg2IhIquSkIRBlekvz5ITHDdtfNaT4VRmvGnqYaE2TOTgtY8oVY7XkDEiEHA6s5zRsWlCyNxNkEgkX4KYm4__
//param_opt_3: 1
//card_pay_method: MPI
//param_opt_2: 2
//buyr_name: 데이터크레딧
//good_name: 망고아이 수강신청
//card_code: CCDI
//card_mask_no: 4032930000000000
//pay_method: CARD
//smart_useyn: Y
//res_cd: 0000
//kcp_select_card_code: CCDI
//use_pay_method: 100000000000
//enc_data: 2tLgG6812o2.41WusgKP1AYEv5zi5OV0IkdcOuqXpE5sdYnTfzR8wx.SIDGScwO09OxyI77Wpvt.eTINzZHsKj.36JwO064UPWDrNVavywfsqYIiHEBF8HRo9ocHVgBCAuJSsOxb4ZLBwwEDWqmUdBlqTwS9lszJLUyDVrur3n4pbqH2SmyibclqJ0ELXk8uk7bzmaBdGHP9BkUhSrTEGc9RPh1FrZXw.XfbT6aJxQYk25TVkjh2Q3kZ7E3rCzpdVjt30MwiLN.v-TaDr6tRXXUaHXL7PKI1pWFrzPYTO5W-J3NMcuTIGhIiX-zhzNbeekh-s-3Iak-g8XjqaFVs-KhaWhHxwQhShkbpUrnmkGbdDLPyfvV7q1TtZgjuvxJXfJxnbLiGky41-0q41cIZy7izwdBOXbYKdXJXvtfG0a7o2ZHv0gRbeHM0h0EdNb1-RcSnJNnoSPmpTHDVYoJxEsuKxVH2krxSqg.IhDQ1lfVmxFpi2doC.H.TqqAxNHpGDLbW.YPguHSAm.OU1VVXda8DmUwDhfwXe.-gULyHf.VYW8LCvJ2xNGOKMktIqyTibLc.QKAG30Xc0rgkNk-o22AN2uIWgatbQMNE8S2i3ImjTamvzaStVhvKn38T1NL5whXG8w8Y5kWw7R9Hjdiu4n16j.OFhJFcfCSx4lPO4.XXTsR1YCv5wJzsbRbyxzrLtgRj9MN4EVaKQM6kgjalPjgQZ4NACFmO8GEgd0YKvSKb019OLJN.hQQcnLJSGzoxdFzR.aeq3QDUCL1X4ceNMB6xaWcxtHzAd5dz.59g1-Z1yV0SpiUEqrFVUM0R3dmU5WIXqvIbsQ8z65z1QjFU1DyRVExKGBZg67g-3iIWOx3dSDe0YVrBWGuMftQ0b5OfIeSW2DaxGjZ80mhbF1m8dhHV3ssGjNb2wtOPEStvWwzv_
//good_mny: 2000
//site_cd: T0000
//Ret_URL: https://www.selfpay.kr/rdvDN15
//ordr_idxx: MH202404080254360000092338-1
...
function isMobile(): bool
{
	...
	$mobileKeyWords = array(
		...
		'SonyEricsson', 'Mobile', 'HTC', 'Palm', 'Opera Mini', 'Nokia', 'Fennec', 'Silk',
		'webOS', 'UCWEB', 'IEMobile', 'Sony', 'Bada', 'Kindle', 'Symbian', 'BB10', 'PlayBook',
		'Tizen', 'Nexus', 'Huawei', 'Xiaomi', 'Oppo', 'Vivo', 'Motorola', 'OnePlus'
	);
...
// 접속 기기가 모바일일 경우
if (isMobile()) {
	$PayMethod = isset($_REQUEST["pay_method"]) ? $_REQUEST["pay_method"] : "";       //' 모방일 결제타입 : CARD, BANK
	$PayCustName = isset($_REQUEST["buyr_name"]) ? $_REQUEST["buyr_name"] : "";      //' 구매자명
	$PayGoods = isset($_REQUEST["good_name"]) ? $_REQUEST["good_name"] : "";      //' 구매상품명
	$PayMny = isset($_REQUEST["good_mny"]) ? $_REQUEST["good_mny"] : "";      //' 구매결제금액
	$OrderNumPay = isset($_REQUEST["ordr_idxx"]) ? $_REQUEST["ordr_idxx"] : "";      //' 주문번호
	$PayTradenum = isset($_REQUEST["ordr_idxx"]) ? $_REQUEST["ordr_idxx"] : "";      //' 주문번호
	$PayResultCd = isset($_REQUEST["res_cd"]) ? $_REQUEST["res_cd"] : "";      //' 결과코드(성공 : 0000)
	$PayResultMsg = isset($_REQUEST["res_msg"]) ? $_REQUEST["res_msg"] : "";      //' 결제결과메시지
	$PayReTrno = isset($_REQUEST["trace_no"]) ? $_REQUEST["trace_no"] : "";      //' PG사 거래번호(취소시 필요한번호)
	$PayReNum = isset($_REQUEST["app_no"]) ? $_REQUEST["app_no"] : "";      //' PG사 거래승인번호						?
	$PayReTime = isset($_REQUEST["app_time"]) ? $_REQUEST["app_time"] : "";      //' PG사 거래일시						?
	$PayCardCD = isset($_REQUEST["card_code"]) ? $_REQUEST["card_code"] : "";      //' 카드사 코드
	$PayCard = isset($_REQUEST["card_name"]) ? $_REQUEST["card_name"] : "";      //' 카드사명							?
	$PayDivMon = isset($_REQUEST["quota"]) ? $_REQUEST["quota"] : "";      //' 할부 개월수								?
	$PayBankCD = isset($_REQUEST["bank_code"]) ? $_REQUEST["bank_code"] : "";      //' 결제은행코드					?
	$PayBank = isset($_REQUEST["bank_name"]) ? $_REQUEST["bank_name"] : "";      //' 결제은행							?
	$PayCashYN = isset($_REQUEST["cash_yn"]) ? $_REQUEST["cash_yn"] : "";      //' 현금영수증사용여부					?
	$PayReqURL = isset($_REQUEST["Ret_URL"]) ? $_REQUEST["Ret_URL"] : "";      //' ReqURL							?
//	$PayReTime = isset($_REQUEST["app_time"]) ? $_REQUEST["app_time"] : "";
//	$PayCardCD = isset($_REQUEST["card_code"]) ? $_REQUEST["card_code"] : "";
//	$PayCard = isset($_REQUEST["card_name"]) ? $_REQUEST["card_name"] : "";
//	$PayDivMon = isset($_REQUEST["quota"]) ? $_REQUEST["quota"] : "";
//	$PayBankCD = isset($_REQUEST["bank_code"]) ? $_REQUEST["bank_code"] : "";
//	$PayBank = isset($_REQUEST["bank_name"]) ? $_REQUEST["bank_name"] : "";
//	$PayCashYN = isset($_REQUEST["cash_yn"]) ? $_REQUEST["cash_yn"] : "";
//	$PayReqURL = isset($_REQUEST["Ret_URL"]) ? $_REQUEST["Ret_URL"] : "";
//
//	$PayMethod = isset($_REQUEST["pay_method"]) ? $_REQUEST["pay_method"] : "";       //' 모방일 결제타입 : CARD, BANK
//	$PayCustName = isset($_REQUEST["buyr_name"]) ? $_REQUEST["buyr_name"] : "";      //' 구매자명
//	$PayGoods = isset($_REQUEST["good_name"]) ? $_REQUEST["good_name"] : "";      //' 구매상품명
...
//	$PayReqURL = isset($_REQUEST["Ret_URL"]) ? $_REQUEST["Ret_URL"] : "";      //' ReqURL
//
//// 초기화되지 않은 변수들을 추가적으로 초기화
//	$ClassOrderPayPgFeeRatio = isset($ClassOrderPayPgFeeRatio) ? $ClassOrderPayPgFeeRatio : 0;
//	$ClassOrderPayPgFeePrice = isset($ClassOrderPayPgFeePrice) ? $ClassOrderPayPgFeePrice : 0;
//	$ClassOrderPayUseCashPaymentType = isset($ClassOrderPayUseCashPaymentType) ? $ClassOrderPayUseCashPaymentType : 0;
//	$PayMethod = isset($PayMethod) ? $PayMethod : "";
//	$PayCustName = isset($PayCustName) ? $PayCustName : "";
//	$PayGoods = isset($PayGoods) ? $PayGoods : "";
//	$PayMny = isset($PayMny) ? $PayMny : "";
//	$OrderNumPay = isset($OrderNumPay) ? $OrderNumPay : "";
//	$PayTradenum = isset($PayTradenum) ? $PayTradenum : "";
//	$PayResultCd = isset($PayResultCd) ? $PayResultCd : "";
//	$PayResultMsg = isset($PayResultMsg) ? $PayResultMsg : "";
//	$PayReTrno = isset($PayReTrno) ? $PayReTrno : "";
//	$PayReNum = isset($PayReNum) ? $PayReNum : "";
//	$PayReTime = isset($PayReTime) ? $PayReTime : "";
//	$PayCardCD = isset($PayCardCD) ? $PayCardCD : "";
//	$PayCard = isset($PayCard) ? $PayCard : "";
//	$PayDivMon = isset($PayDivMon) ? $PayDivMon : "";
//	$PayBankCD = isset($PayBankCD) ? $PayBankCD : "";
//	$PayBank = isset($PayBank) ? $PayBank : "";
//	$PayCashYN = isset($PayCashYN) ? $PayCashYN : "";
//	$PayReqURL = isset($PayReqURL) ? $PayReqURL : "";
...
if (!$PrePayClassOrderPayID){
	...
	if ($PayResultCd == "0000"){
		...
		if ($ClassOrderPayUseSavedMoneyPrice > 0){
			InsertNewPoint(1, 1, $MemberID, '결제 차감', '수강 신청 결제 사용', ($ClassOrderPayUseSavedMoneyPrice * -1));
		}	
		...
		*/
		...
		$msg = "$MemberName 님의 수강료 결제가 완료되었습니다. 망고아이와 즐거운 수업 되세요. 감사합니다.";
			
		$tmplId="mangoi_007";  //카카오 알림톡 템플릿 아이디(문자 메시지를 바꿀려면 비즈엠에서 템플릿 등록하고 그 메시지를 위에 복사후 템플릿id도 바꿔야 함)

...
while($Row = $Stmt->fetch()) {
	...
	if($DirectQnaMemberState == 1) {
		$StrDirectQnaMemberState = "<trn class='TrnTag'>답변대기중</trn>";
	} else if($DirectQnaMemberState == 2) {
		$StrDirectQnaMemberState = "<a href=\"#\" class=\"mantoman_btn TrnTag\">답변보기 <span class=\"mantoman_arrow \"></span></a>";
	}
	...
	if($DirectQnaMemberState == 1) {
		$PageDirectQnaListHTML .= "			<div class=\"mtm_reply_state ing TrnTag\">답변대기</div>";
	}else{
		$PageDirectQnaListHTML .= "			<div class=\"mtm_reply_state complete TrnTag\">답변완료</div>";
	}	
	$PageDirectQnaListHTML .= "		</div>";

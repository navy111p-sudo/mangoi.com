...
if ($BoardContentID!=""){
	...
	$SupportOrganID = $Row["SupportOrganID"];
	//============= 추가항목 
	...
	<div id="bbs">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="TbWriteTop">
		  ...
		  <tr>
			<th>지원항목</th>
			<td colspan="2">
					<select name="SupportItemID"  class="Select Font3">
						...
						while($Row2 = $Stmt2->fetch()){
						?>
						<option value="<?=$Row2["SupportItemID"]?>" <?if ($SupportItemID==$Row2["SupportItemID"]) {?>selected<?}?>><?=$Row2["SupportItemName"]?></option> 
						<?
						}
						$Stmt2 = null;
						?>
		  ...
		  <tr>
			<th>지원기관</th>
			<td colspan="2">
					<select name="SupportOrganID" class="Select Font3">
						...
						while($Row2 = $Stmt2->fetch()){
						?>
						<option value="<?=$Row2["SupportOrganID"]?>" <?if ($SupportOrganID==$Row2["SupportOrganID"]) {?>selected<?}?>><?=$Row2["SupportOrganName"]?></option> 
						<?
						}
						$Stmt2 = null;
						?>
...
function FormSubmit(){
	...
	obj = document.RegForm.SupportItemID;
	if (obj.value==""){
		alert('지원항목을 선택하세요.');
		obj.focus();
		return;
	...
	obj = document.RegForm.SupportOrganID;
	if (obj.value==""){
		alert('지원기관을 선택하세요.');
		obj.focus();
		return;

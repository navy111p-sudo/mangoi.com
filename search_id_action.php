...
$Sql = "select count(*) as TotalRowCount from Members where MemberState=1 and MemberName=:MemberName and MemberEmail=HEX(AES_ENCRYPT(:MemberEmail, :EncryptionKey))";
...
if ($TotalRowCount==0){
	...
	$Sql = "select * from Members where MemberState=1 and MemberName=:MemberName and MemberEmail=HEX(AES_ENCRYPT(:MemberEmail, :EncryptionKey))";
	$Stmt = $DbConn->prepare($Sql);
	$Stmt->bindParam(':MemberName', $MemberName);
	$Stmt->bindParam(':MemberEmail', $MemberEmail);
	$Stmt->bindParam(':EncryptionKey', $EncryptionKey);
	...
	<div style=\"overflow:hidden; border-bottom:2px solid #111; padding-bottom:30px; letter-spacing:-0.5px;\">
        <h2 style=\"font-family:'맑은 고딕', '나눔고딕', '돋움'; font-size:23px; padding:0; margin:0; color:#303030; text-align:left; float:left; padding-top:16px;\">아이디 정보를 알려드립니다.
            <p style=\"font-size:14px; padding:4px 0 0 0; margin:0; color:#4e4e4e;\">요청하신 아이디 정보를 안내드립니다.</p>
        </h2>
    ...
    <div style=\"font-size:14px; padding:40px 0; border-bottom:1px solid #ccc;\">
		...
		회원님의 아이디는 <u><b>".$MemberLoginID."</b></u>입니다. 
    </div>

    ...
    function _buildBody () {
      ...
      for (var k = 0; k < $.FE.DEFAULTS.specialCharactersSets.length; k++) {
        ...
        for (var i = 0; i < list.length; i++) {
          var item = list[i];
          html_list += '<span class="fr-command fr-special-character" tabIndex="-1" role="button" value="' + item['char'] + '" title="' + editor.language.translate(item.desc) + '">' + item['char'] + '<span class="fr-sr-only">' + editor.language.translate(item.desc) + '&nbsp;&nbsp;&nbsp;</span></span>';

...
<div class="sub_wrap bg_gray padding_app" style="border:0;<?if ($FromDevice==""){?>margin-top:-60px;<?}?>">   
    <section class="level_application_wrap">
        <div class="level_application_area">
			...
			<div class="level_application_text_box" style="display:inline-block;margin-bottom:30px;width:49%;cursor:pointer;background-color:#cccccc;" onclick="ChResetSelectType(2)">
				강사먼저 선택
            </div>
					...
					<table class="lms_teacher_time_table">
						...
						<tr>
							<td>
								...
								<select name="StudyTimeHour2" id="StudyTimeHour2" class="lms_teacher_time_table_select" onchange="GetTeacherList()">
									<?for ($ii=9;$ii<=23;$ii++){?>
									<option value="<?=$ii?>" <?if ($SetHour==$ii){?>selected<?}?>><?=substr("0".$ii,-2)?></option>
									<?}?>
								</select> 시
								<select name="StudyTimeMinute2" id="StudyTimeMinute2" class="lms_teacher_time_table_select" onchange="GetTeacherList()" style="margin-left:10px;">
...
function GetTeacherList(){
	...
	document.getElementById("DivTeacherListHTML").innerHTML = "<div style=\"width:100%;text-align:center;margin-top:50px;\"><img src=\"images/loading.gif\"></div>";

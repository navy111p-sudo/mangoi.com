...
$memberSql = "SELECT MemberLoginID FROM Members where MemberID = :MemberID";
$Stmt = $DbConn->prepare($memberSql);
...
while($Row = $Stmt->fetch() ) {
	...
	if($FavoriteMenuListType == 2){
		$path = str_replace("'","",$Row["FavoriteMenuListPath"]);
		$FavoriteMenuListPath = "'{$path}&TempLinkLoginMemberID={$memberRow['MemberLoginID']}'";
	}else{

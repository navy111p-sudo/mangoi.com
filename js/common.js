$(document).ready(function () {
	...
	$('.toggle_navi').mouseenter(function(e){
		e.preventDefault();
		$(this).find('.sub_navi').stop().slideDown(200);
	});
	$('.toggle_navi').mouseleave(function(e){
		e.preventDefault();
		$(this).find('.sub_navi').stop().slideUp(200);
	});
	...
	// 샘플비디오 선택
	var SampleChoose=0;
	$('.feature_sample_btn.choose').click(function(e){
		e.preventDefault();
		if(SampleChoose==0){			
			$('.feature_sample_select_list').slideDown(300);			
			SampleChoose=1;
		}else{
			$('.feature_sample_select_list').slideUp(300);
			SampleChoose=0;
		}
	});

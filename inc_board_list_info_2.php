	...
	<div id="bbs">
		...
		<ul class="Display">
			...
			while($Row = $Stmt->fetch()) {
				...
				if ($BoardContentPrice==""){
					$BoardContentPrice = "-";
				}

				$BoardContentHelp = trim($Row["BoardContentHelp"]);

			?>
			  <li>
				<a>
					<div class="txt">
						<h4><?if ($AuthWrite){?><a href="<?=$LinkUrl?>"><?}?><?=$Row["BoardContentSubject"]?><?if ($AuthWrite){?></a><?}?></h4>
						<div class="content">
							<?=$Row["BoardContent"]?>
						</div>
						<div class="price">가격 : <?=$BoardContentPrice?></div>
					</div>
					<div class="download">
					<h4>다운로드</h4>
					<?php
					...
					while($Row2 = $Stmt2->fetch()){
					?>
					<strong><a href="board_file_down.php?BoardFileID=<?=$Row2["BoardFileID"]?>"><?=$Row2["BoardFileRealName"]?></a></strong><br><br>
					<?php
					}
					$Stmt2 = null;
					?>
					...
					<?if ($BoardContentHelp!=""){?>
					<strong><a href="javascript:OpenHtml('<?=$BoardContentHelp?>')" style="color:#486AF4;">사용설명서</a></strong><br><br>
					<?}?>
					</div>
				</a>
			  </li>
...
<script>
function OpenHtml(HtmlFile){
	window.open(HtmlFile, "_blank", "toolbar=no,scrollbars=yes,resizable=yes,top=100,left=100,width=1000,height=800");
}

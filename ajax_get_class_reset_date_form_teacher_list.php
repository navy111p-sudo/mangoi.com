...
$ResetMonth_2 = date('n', strtotime($StudyTimeDate));
if (($ResetMonth_2-1)==0){
	$PrevLinkYear = $ResetYear - 1;
	$PrevLinkMonth = 12;
}else{
	$PrevLinkYear = $ResetYear;
	$PrevLinkMonth = $ResetMonth_2-1;
}
if (($ResetMonth_2+1)==13){ 
	$NextLinkYear = $ResetYear + 1;
	$NextLinkMonth = 1;
}else{
	$NextLinkYear = $ResetYear;
	$NextLinkMonth = $ResetMonth_2+1;
}
...
while($Row = $Stmt->fetch()) {
	...
	if ($SearchTeacherID==""){//강사 아이디가 있을때는 내용만 가져다 뿌림 ===================================================
		...
		$TeacherListHTML .= "		<a href=\"#\" class=\"level_teacher_select_btn\" onclick=\"OpenTeacherCalendar(".$TeacherID.", ".$TeacherBlock80Min.")\">강사선택 <span class=\"level_teacher_select_arrow\"></span></a>";
		$TeacherListHTML .= "	</div>";
	...
	if ($SearchTeacherID!=""){//강사 아이디가 있을때만 슬랏 검색
		...
		$TeacherListHTML .= "				".$ResetYear.".".$ResetMonth." ";
		...
		for ($nn=1,$ii=0; $ii<$tweek; $ii++){
			...
			for ($kk=0; $kk<7; $kk++){
				...
				if ($today-$NowDate2==0 && $ResetTimeWeek==$kk){
					$UseLink = 0;
		...
		$TeacherListHTML .= "			<tr>";
		$TeacherListHTML .= "				<th>".str_replace("-",".",$StudyTimeDate)." (".$WeekDayStr.")</th>";
		$TeacherListHTML .= "			</tr>";
		...
		if ($EduCenterHoliday[$WeekDayNum]==0){
			...
			for ($HourNum=$TeacherStartHour;$HourNum<$TeacherEndHour;$HourNum++){
				...
				for ($MinuteNum=0;$MinuteNum<=50;$MinuteNum=$MinuteNum+10){
					...
					$HourNum_2 = $HourNum;
					if ($HourNum_2<=12){
						$HourNum_2 = "am ".substr("0".$HourNum_2,-2);
					}else{
						$HourNum_2 = "pm ".substr("0".($HourNum_2-12),-2);
					}
					...
					if ($EduCenterBreak[$WeekDayNum][$HourNum][$MinuteNum]==1 && $TeacherBreak[$TeacherNum][$WeekDayNum][$HourNum][$MinuteNum]==1 && $SlotBreakEvent[$TeacherNum][$WeekDayNum][$HourNum][$MinuteNum]==0){
						...
						$TeacherListHTML .= "<span id=\"Div_Slot_".$TeacherID."_".$WeekDayNum."_".$MinuteListNum."\" class=\"teacher_time\" onclick=\"SelectSlot(".$TeacherID.",".$HourNum.",".$MinuteNum.", ".$WeekDayNum.", ".$MinuteListNum.", ".$TeacherBlock80Min.");\" style=\"display:inline-block;width:88px;margin-right:2px;\">".$HourNum_2.":".substr("0".$MinuteNum,-2)."";
						...
						$TeacherListHTML .= "<span id=\"Div_Slot_".$TeacherID."_".$WeekDayNum."_".$MinuteListNum."\" class=\"teacher_time\" style=\"background-color:#bbbbbb;cursor:default;display:none;\">".$HourNum_2.":".substr("0".$MinuteNum,-2)."";
		...
		$TeacherListHTML .= "		</table>";
		$TeacherListHTML .= "		<a href=\"javascript:OpenResetReserveSubmit();\" class=\"teacher_select_submit\">".$StrResetType." 하기</a>";

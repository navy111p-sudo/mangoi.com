...
if ($use_pay_method=="001000000000"){//가상계좌
	...
	$OrderPayPgFeePrice = $OnlineSitePgVBankFeePrice;
...
if (!$PreProductOrderID){
	...
	$Sql_Log = " update ProductOrders set ";
		...
		if ($PayResultCd == "0000"){
			if ($use_pay_method=="001000000000"){//가상계좌
				$Sql_Log .= " ProductOrderState = 11, ";
			}else{
				$Sql_Log .= " ProductOrderState = 21, ";
				$Sql_Log .= " PaymentDateTime = now(), ";
			}
			$Sql_Log .= " UseCashPaymentType = '$UseCashPaymentType', ";
	...
	$Sql = "update ProductOrders set ";
		...
		$Sql .= " LastUpdateUrl = 'product_order_pay_result.php', ";
...
$FromMyPage =  isset($_COOKIE["FromMyPage"]) ? $_COOKIE["FromMyPage"] : "";
$FromMyPageMemberID =  isset($_COOKIE["FromMyPageMemberID"]) ? $_COOKIE["FromMyPageMemberID"] : "";
setcookie("FromMyPage", "", 0, "/", ".".$DefaultDomain2);
setcookie("FromMyPageMemberID", "", 0, "/", ".".$DefaultDomain2);


if ($FromMyPage=="Y"){
	header("Location: ./mypage/login_action_from_pay.php?FromMyPageMemberID=$FromMyPageMemberID"); 
	exit;
}else{
	header("Location: product_order_list.php"); 
	exit;
}
?> 
						...
						<div class="uk-margin-top" style="margin-top:20px;">
							<div class="uk-grid" data-uk-grid-margin>
								<div class="uk-width-medium-6-10">
									<label for="SendTitle">메시지 제목</label>
						...
						<div class="uk-margin-top">
							<div class="uk-grid" data-uk-grid-margin>
								<div class="uk-width-medium-1-1">
									...
									<textarea class="md-input" name="SendMessage" onkeyup="CheckByte(this.value);"  id="SendMessage" cols="30" rows="4"><?=$SendMessage?></textarea>
								</div>
							</div>
							<div style="float: right; margin-top: 3px; " ><span id="modifyByte">0</span> / 90 Byte</div>
						</div>
...
function CheckByte(str) {
	var bytes = 0;
	var modifyByte = document.getElementById("modifyByte");

	for (var i=0; i<str.length; ++i) {

	// 기본 한글 2바이트 처리

	(str.charCodeAt(i) > 127) ? bytes += 2 : bytes++ ;

	}

	modifyByte.innerHTML = bytes;
}
...
function FormSubmit(){
	...
	UIkit.modal.confirm(
		'저장 하시겠습니까?', 
		function(){ 
			document.RegForm.action = "send_message_log_multi_action.php";

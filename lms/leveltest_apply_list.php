...
if ($SearchState=="11"){//스케줄대상
	$SubMenuID = 1501;
}else if ($SearchState=="21"){//스케줄완료
	$SubMenuID = 1502;
}else if ($SearchState=="51"){//종료대상
	$SubMenuID = 1505;
}else if ($SearchState=="61"){//종료완료
	$SubMenuID = 1506;
}
...
if ($SearchState!="0"){
	if ($SearchState=="31"){
		$AddSqlWhere = $AddSqlWhere . " and A.ClassOrderState=5 and A.ClassProgress=11 ";//테스트완료
	}else if ($SearchState=="41"){
		$AddSqlWhere = $AddSqlWhere . " and A.ClassOrderState=6 and A.ClassProgress=11 ";//미응시
...
$AddSqlWhere = $AddSqlWhere . " and A.ClassProductID=2 ";
...
$Sql = "
		select 
			...
			G.FranchiseName,
			ifnull((select concat(ClassOrderSlotDate, ' (', StudyTimeHour, '시 ' , StudyTimeMinute, '분',')' ) from ClassOrderSlots where ClassOrderID=A.ClassOrderID order by StudyTimeHour asc, StudyTimeMinute asc limit 0,1),'-') as LeveltestDateTime,
			ifnull((select BBB.TeacherName from ClassOrderSlots AAA inner join Teachers BBB on AAA.TeacherID=BBB.TeacherID where AAA.ClassOrderID=A.ClassOrderID order by AAA.StudyTimeHour asc, AAA.StudyTimeMinute asc limit 0,1),'-') as LeveltestTeacherName
		from ClassOrders A
			inner join ClassProducts AA on A.ClassProductID=AA.ClassProductID 
			...
			inner join Franchises G on F.FranchiseID=G.FranchiseID 
		where ".$AddSqlWhere." 
		order by A.ClassOrderRegDateTime desc limit $StartRowNum, $PageListNum";
							...
							<table class="uk-table uk-table-align-vertical">
								...
								<tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										$LeveltestDateTime = $Row["LeveltestDateTime"];
										$LeveltestTeacherName = $Row["LeveltestTeacherName"];
										...
										}else if ($ClassOrderState==5){
											$StrClassOrderState = $테스트완료[$LangID];
										}else if ($ClassOrderState==6){
											$StrClassOrderState = $미응시[$LangID];
										}
									...
									<tr>
										...
										<td class="uk-text-nowrap uk-table-td-center"><a class="md-btn md-btn-primary md-btn-small md-btn-wave-light" href="javascript:OpenClassOrderForm(<?=$ClassOrderID?>);"><?=$ClassProductName?></a></td>
										...
										<td class="uk-text-nowrap uk-table-td-center">
											...
											<?}else{?>
											<?=$LeveltestDateTime?> / <?=$LeveltestTeacherName?>
											<?}?>
										</td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$StrClassProgress?></td>
...
function OpenClassScheduleForm(ClassOrderID){
	...
	window.open(openurl, "class_order_schedule_form", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,left=50,width=1080,height=800");
}
...
function OpenClassOrderForm(ClassOrderID){
	openurl = "class_order_form.php?ClassOrderID="+ClassOrderID;
	$.colorbox({	
		href:openurl
		,width:"95%" 
		,height:"95%"
...
function SearchSubmit(){
	document.SearchForm.action = "leveltest_apply_list.php";

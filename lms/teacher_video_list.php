...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$강사관리[$LangID]?></h3>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
						<div class="uk-overflow-container">
							...
							?>
								<div style="display:inline-block;border:1px solid #cccccc;width:200px;height:170px;margin-top:20px;margin-right:20px;padding:20px;">
									<div id="YoutubeBgImg_<?=$TeacherID?>" style="width:200px;height:150px;background-size:cover;cursor:pointer;background-image:url()" onclick="OpenVideoPlayer('<?=$TeacherVideoCode?>', <?=$TeacherVideoType?>)"></div>
									<div style="margin-top:10px;"><a href="teacher_form.php?ListParam=<?=$ListParam?>&TeacherID=<?=$TeacherID?>"><?=$TeacherName?>(<?=$MemberLoginID?>)</a></div>
								</div>
								<?if ($TeacherVideoType==1){?>
									<script>
										var url = "http://www.youtube.com/watch?v=<?=$TeacherVideoCode?>";
										var imgUrlbig = getScreen(url);
										document.getElementById("YoutubeBgImg_<?=$TeacherID?>").style.backgroundImage  = "url('"+imgUrlbig+"')";
									</script>
								<?}?>
							<?php
								$ListCount ++;
...
function SearchSubmit(){
	document.SearchForm.action = "teacher_video_list.php";
	document.SearchForm.submit();
}


function OpenVideoPlayer(TeacherVideoCode, TeacherVideoType) {

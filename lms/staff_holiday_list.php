...
$PaginationParam = $ListParam . "&SearchYear=" . $SearchYear;
if ($CurrentPage!=""){
...
<div id="page_content">
	...
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$직원휴가관리[$LangID]?> 
			...
			<select id="SearchYear" name="SearchYear" class="uk-width-1-4" onchange="SearchSubmit()" data-md-select2 data-allow-clear="true" data-placeholder="<?=$년도선택[$LangID]?>"/>
				...
				<?
				for ($yearCount=2018; $yearCount<=((int)date("Y")+1); $yearCount++) {
				?>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
							...
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										...
										<th nowrap width=12%>휴가적용 시작일</th>
										<th nowrap width=12%>1년 최대 휴가일</th>
										<th nowrap width=12%>1년 최대 병가일</th>
									</tr>
								</thead>
								<tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										// 현재 설정되어 있는 휴가적용 시작일과 최대 휴가일 수를 가져온다.
										$Sql2 = "SELECT StartDate, MaxHoliday, MaxSickLeave
													from StaffHoliday 
													where StaffID = $StaffID and Year = $SearchYear";
										$Stmt2 = $DbConn->prepare($Sql2);
										$Stmt2->execute();
										$Stmt2->setFetchMode(PDO::FETCH_ASSOC);
										$Row2 = $Stmt2->fetch();
										...
										$MaxSickLeave = isset($Row2["MaxSickLeave"]) ? $Row2["MaxSickLeave"] : 0;
						
									?>
									<tr>
										<td class="uk-text-nowrap uk-table-td-center"><?=$ListNumber?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$StrStaffManageMent?></td>
										<td class="uk-text-nowrap uk-table-td-center"><a href="staff_personal_holiday_list.php?StaffID=<?=$StaffID?>&SearchYear=<?=$SearchYear?>&MaxHoliday=<?=$MaxHoliday?>&StaffName=<?=$StaffName?>"><?=$StaffName?></a></td>
										<td class="uk-text-nowrap uk-table-td-center"><a href="staff_personal_holiday_list.php?StaffID=<?=$StaffID?>&SearchYear=<?=$SearchYear?>&MaxHoliday=<?=$MaxHoliday?>&StaffName=<?=$StaffName?>"><?=$MemberLoginID?></a></td>
										...
										<td class="uk-text-nowrap uk-table-td-center"><input style="width:140px" type="input" id="StartDate_<?=$ListCount?>" name="StartDate_<?=$ListCount?>" value="<?=$StartDate?>" class="draft_input" style="width:40%;display:inline" data-uk-datepicker="{format:'YYYY-MM-DD', weekstart:0}" > </td>
										<td class="uk-text-nowrap uk-table-td-center">
											<input type="hidden" name="StaffID_<?=$ListCount?>" value="<?=$StaffID?>">
											<input type="input" name="StaffHoliday_<?=$ListCount?>" value="<?=$MaxHoliday?>" class="draft_input" style="display:unset;width:80%">일</td>
										<td class="uk-text-nowrap uk-table-td-center">
											<input type="input" name="StaffSickLeave_<?=$ListCount?>" value="<?=$MaxSickLeave?>" class="draft_input" style="display:unset;width:80%">일</td>
									</tr>
									<?php
										$ListCount ++;
									}
						...
						<div class="uk-form-row" style="text-align:center;">
							<a type="button" href="javascript:HolidaySubmit();" class="md-btn md-btn-primary">최대 휴가일 저장</a>
						</div>
...
function SearchSubmit(){
	document.SearchForm.action = "staff_holiday_list.php";
	document.SearchForm.submit();
}
function HolidaySubmit(){

	url = "ajax_set_staff_holiday.php";
	...
	$.ajax(url, {
		data: {
			...
			?>		
			StaffID_<?=$i?>: $('input[name=StaffID_<?=$i?>]').val(),
			StartDate_<?=$i?>: $('input[name=StartDate_<?=$i?>]').val(),
			StaffHoliday_<?=$i?>: $('input[name=StaffHoliday_<?=$i?>]').val(),
			StaffSickLeave_<?=$i?>: $('input[name=StaffSickLeave_<?=$i?>]').val(),
			<?
				}
			?>
		},

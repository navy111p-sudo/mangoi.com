...
if ($SearchState!="100"){
	...
	$AddSqlWhere = $AddSqlWhere . " and A.TeacherGroupState=$SearchState ";
}
$AddSqlWhere = $AddSqlWhere . " and A.TeacherGroupState<>0 ";
$AddSqlWhere = $AddSqlWhere . " and B.EduCenterState<>0 ";
$AddSqlWhere = $AddSqlWhere . " and C.FranchiseState<>0 ";

if ($SearchText!=""){
	$ListParam = $ListParam . "&SearchText=" . $SearchText;
	$AddSqlWhere = $AddSqlWhere . " and (A.TeacherGroupName like '%".$SearchText."%' or A.TeacherGroupManagerName like '%".$SearchText."%') ";
}
...
$TeacherCountSql = ",(
				...
				where AA.TeacherState<>0 and BB.TeacherGroupID=A.TeacherGroupID 
				) as TeacherCount 
				";
...
$Sql = "
		...
		from TeacherGroups A 
			...
			inner join Franchises C on B.FranchiseID=C.FranchiseID 
		where ".$AddSqlWhere." 
		order by A.TeacherGroupOrder desc limit $StartRowNum, $PageListNum";
$Stmt = $DbConn->prepare($Sql);
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$강사그룹관리[$LangID]?></h3>
			...
			<div class="md-card-content">
				<div class="uk-grid" data-uk-grid-margin="">
					...
					<div class="uk-width-medium-2-10">
						<label for="SearchText"><?=$강사그룹명_또는_관리자명[$LangID]?></label>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
								...
								<tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										if ($TeacherGroupState==1){
											$StrTeacherGroupState = "<span class=\"ListState_1\"><?=$운영중[$LangID]?></span>";
										}else if ($TeacherGroupState==2){
											$StrTeacherGroupState = "<span class=\"ListState_2\"><?=$미운영[$LangID]?></span>";
										}
									?>
									<tr>
										<td class="uk-text-nowrap uk-table-td-center"><?=$ListNumber?></td>
										<td class="uk-text-nowrap uk-table-td-center"><a href="teacher_group_form.php?ListParam=<?=$ListParam?>&TeacherGroupID=<?=$TeacherGroupID?>"><?=$TeacherGroupName?></a></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$TeacherGroupManagerName?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$TeacherGroupPhone1?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$TeacherGroupPhone2?></td>
										...
										<td class="uk-text-nowrap uk-table-td-center"><?=$StrTeacherGroupState?></td>
									</tr>
									<?php
										$ListCount ++;
									}
						...
						<div class="uk-form-row" style="text-align:center;">
							<a type="button" href="teacher_group_form.php?ListParam=<?=$ListParam?>" class="md-btn md-btn-primary"><?=$신규등록[$LangID]?></a>
						</div>
...
function SearchSubmit(){
	document.SearchForm.action = "teacher_group_list.php";

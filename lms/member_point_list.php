...
if ($SearchState!="100"){
	...
	$AddSqlWhere = $AddSqlWhere . " and A.MemberPointState=$SearchState ";
}
$AddSqlWhere = $AddSqlWhere . " and A.MemberPointState<>0 ";
...
$Sql = "select 
		...
		where ".$AddSqlWhere." and A.MemberPointState=1 ";
...
$Sql = "
		...
		where ".$AddSqlWhere." and A.MemberPointState=1 and A.MemberPointTypeID<>3 
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$포인트관리[$LangID]?></h3>
					...
					<div class="uk-width-medium-1-10" style="display:none;">
						<div class="uk-margin-small-top">
							<select id="SearchState" name="SearchState" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
								...
								<option value="2" <?if ($SearchState=="2"){?>selected<?}?>><?=$비정상[$LangID]?></option>
							</select>
						</div>
					</div>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
							...
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										...
										<th nowrap><?=$적립일시[$LangID]?></th>
										<th nowrap><?=$학생명[$LangID]?>/대리점장명</th>
										<th nowrap><?=$포인트[$LangID]?></th>
										<th nowrap><?=$누적포인트[$LangID]?></th>
								...
								<tbody>
									...
									<tr>
										...
										<td class="uk-text-nowrap uk-table-td-center"><?=$MemberLevelName?></td>
										<td class="uk-text-nowrap"><a href="javascript:OpenMemberPointForm(<?=$MemberPointID?>);"><?=$MemberPointName?></a></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$MemberPointRegDateTime?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$MemberName?> 
											<?if($MemberLevelID==19){?>
												<a href="javascript:OpenStudentForm(<?=$MemberID?>);"><i class="material-icons">account_box</i></a>
											<?} else if($MemberLevelID==12) {?>
												<a href="javascript:OpenCenterSummary(<?=$CenterID?>);"><i class="material-icons">account_box</i></a>
										<?}?><br>(<?=$MemberLoginID?>)</td>
						...
						<div class="uk-form-row" style="text-align:center;display:none;">
							<a type="button" href="member_point_form.php?ListParam=<?=$ListParam?>" class="md-btn md-btn-primary"><?=$신규등록[$LangID]?></a>
						</div>
...
function OpenMemberPointForm(MemberPointID){
	openurl = "member_point_form.php?MemberPointID="+MemberPointID;
	$.colorbox({	
		href:openurl
		,width:"95%" 
		,height:"95%"
...
function SearchSubmit(){
	document.SearchForm.action = "member_point_list.php";

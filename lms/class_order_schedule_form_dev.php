...
<div id="DivLoading" style="position:absolute; top:50%; left:50%; width:200px; height:300px; overflow:hidden; margin-top:-150px; margin-left:-100px;z-index:1000000;">
	...
	<span style="display:block;text-align:center;margin-top:20px;">페이지 로딩중 입니다<span>
</div>
<?php
...
<!-- --------------------------------------------------------------- AA -->
<h3 class="heading_b uk-margin-bottom" style="margin-top:-30px;">수강신청 스케줄관리</h3>
...
<div class="uk-width-xLarge-10-10  uk-width-large-10-10" style="margin-bottom:20px;">
	<div class="md-card" style="padding:20px;">
		...
		<table class="type05" style="width:100%;">
			<tr>
				<th style="width:25%;padding-top:10px;">강사그룹</th>
				<td style="text-align:center;" colspan="3">
					<select id="SearchTeacherGroupID" name="SearchTeacherGroupID" class="uk-width-1-1" onchange="SearchSubmit()" data-md-select2 data-allow-clear="true" data-placeholder="강사그룹선택" style="width:100%;"/>
						...
						$Stmt2 = null;
						?>
						<option value="0" <?if ($SearchTeacherGroupID=="0"){?>selected<?}?>>전체그룹</option>
					</select>	
				</td>
			</tr>
			<tr>
				<th style="width:25%;">수업종류</th>
				<th style="width:25%;">학생명</th>
				<th style="width:25%;">학습시간<?if ($ClassProductID==1){?>/회<?}?></th>
				<th style="width:25%;">학습회수<?if ($ClassProductID==1){?>/주<?}?></th>
			</tr>
			<tr>
			...
			<tr>
				<th style="width:25%;">주간 시작일</th>
				<th style="width:25%;">수업 시작일</th>
				<th style="width:50%;" colspan="2">수업타입</th>
			</tr>
			<tr>
				<td style="text-align:center;">
					<select id="ClassOrderStartDate" name="ClassOrderStartDate" onchange="ChClassOrderStartDate()" class="uk-width-1-1" data-md-select2 data-allow-clear="true" data-placeholder="주간 시작일" style="width:100%;"/>
						<?for ($ii=0;$ii<=3;$ii++) {?>
				...
				<td style="text-align:center;">
					...
					<?if ($ClassProductID!=1){?>
					<div style="padding-top:10px;">자동계산</div>
					<?}?>					
					<span style="display:<?if ($ClassProductID!=1){?>none<?}?>;">
						<select id="ClassOrderRealStartDate" name="ClassOrderRealStartDate" class="uk-width-1-1" data-md-select2 data-allow-clear="true" data-placeholder="수업 시작일" style="width:100%;"/>
							<?
							for ($ii=1;$ii<=5;$ii++) {
...
<div class="uk-form-row" id="submit" style="text-align:center;display:<?if ($ClassProgress!=1){?>none<?}?>;margin-top:20px;">
	<a type="button" href="javascript:ClassOrderSubmit();" class="md-btn md-btn-primary">적용하기</a>
</div>
...
?>	
	...
	if ($("#Able_"+TeacherNum+"_"+WeekDayNum+"_"+MinuteListNum).val()=="1"){
		...
		/*
		if (DenySelect == 0 && TeacherBlock80Min==1){//원하는 시간 모두 선택가능할때 - 이렇게 추가할경우 80분 연속강의에 위배되지 않는지 검사(시간순)
			
			StartListNum = MinuteListNum+ClassOrderTimeSlotCount;
			EndListNum = MinuteListNum+8;
			DenySelect = 2;
			for (ii=StartListNum;ii<=EndListNum;ii++){
				//이중에 한개라도 1이 있으면 통과
				//alert($("#Able_"+TeacherNum+"_"+WeekDayNum+"_"+ii).val());
				if ($("#Able_"+TeacherNum+"_"+WeekDayNum+"_"+ii).length==0 || $("#Able_"+TeacherNum+"_"+WeekDayNum+"_"+ii).val()=="1" || $("#Break_"+TeacherNum+"_"+WeekDayNum+"_"+ii).val()!="11"){
					DenySelect = 0;
				}
			}
		...
		if (DenySelect == 0 && TeacherBlock80Min==1){//원하는 시간 모두 선택가능할때 - 이렇게 추가할경우 80분 연속강의에 위배되지 않는지 검사(역순)
			
			StartListNum = MinuteListNum-1;
			EndListNum = MinuteListNum+ClassOrderTimeSlotCount-9;
			DenySelect = 2;
			for (ii=StartListNum;ii>=EndListNum;ii--){
				//이중에 한개라도 1이 있으면 통과
				//alert($("#Able_"+TeacherNum+"_"+WeekDayNum+"_"+ii).val());
		...
		*/
		...
		if (SelectSlotCode.indexOf(SlotCode)<0 && SelectedSlotCount>=(ClassOrderWeekCount*ClassOrderTimeSlotCount)){
			<?if ($ClassProductID==2 || $ClassProductID==3){?>
				UIkit.modal.alert( "먼저 선택한 시간을 취소한 후 새로운 시간을 선택하세요.");
			<?}else{?>
				UIkit.modal.alert( "주" + ClassOrderWeekCount + "회까지 선택할 수 있습니다.");
			<?}?>
			...
			if (DenySelect==1){
				UIkit.modal.alert( (ClassOrderTimeSlotCount*10) + '분 수업을 구성할 수 없습니다.');
...
function ClassOrderSubmit(){
	if (SelectedSlotCount < (ClassOrderWeekCount*ClassOrderTimeSlotCount)){
		UIkit.modal.alert( "주" + ClassOrderWeekCount + '회 선택하셔야 합니다.');
	}else{
		...
		UIkit.modal.confirm(
			...
			function(){ 
				...
				//location.href = url + "?ClassOrderStartDate="+ClassOrderStartDate+"&ClassOrderRealStartDate="+ClassOrderRealStartDate+"&ClassOrderID="+ClassOrderID+"&ClassProductID="+ClassProductID+"&SelectSlotCode="+SelectSlotCode+"&SelectStudyTimeCode="+SelectStudyTimeCode+"&ClassMemberType=<?=$ClassMemberType?>"+ClassMemberType;;
...
function SearchSubmit(){
	document.SearchForm.action = "class_order_schedule_form_dev.php";
...
function GetScheduleTable(){
	...
	url = "ajax_get_class_order_schedule_table_dev.php";

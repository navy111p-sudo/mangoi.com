...
if ($SearchDate==3) {
    ...
    // 필요시 HideStartDay/HideEndDay 플래그를 설정할 수 있으나, 여기서는 전체 표기
}
if ($SearchDate==3) {
...
// 최종적으로 BranchDivision별 합계를 구함
$Sql = "
SELECT 
    BranchDivision, 
    SUM(TotalStudents) as TotalStudents, 
    SUM(TotalClassOrderPayUseCashPrice) as TotalClassOrderPayUseCashPrice, 
    SUM(TotalBranchFee) as TotalBranchFee
FROM (
    $SubQuery
) T
GROUP BY BranchDivision
ORDER BY BranchDivision ASC
";
...
<div id="page_content">
    <div id="page_content_inner">
        <h3 class="heading_b uk-margin-bottom">커미션 데이터 (지사별 합계)</h3>
...
<script>
    ...
    function ChSearchStartMonth(MonthType, MonthNumber){
        ...
        if(!document.getElementById(daySelectId)) return;
    ...
    function SearchSubmit(){
        ...
        document.SearchForm.action = "commision_branch_summary.php";
        document.SearchForm.submit();
    }
    // 페이지 로딩 완료 후, 1초마다 document.readyState를 체크하여 오버레이 숨김

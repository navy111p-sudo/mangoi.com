...
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and TCOS.TeacherID>0 ";
if ($SearchTeacherID!=""){
	$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and TCOS.TeacherID=".$SearchTeacherID." ";
}
...
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and concat(lpad(TCOS.StudyTimeHour,2,0) , lpad(TCOS.StudyTimeMinute,2,0))>=$SearchStartHourMinute ";
...
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and concat(lpad(TCOS.StudyTimeHour,2,0) , lpad(TCOS.StudyTimeMinute,2,0))<=$SearchEndHourMinute ";
//============================
...
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and TCOS.ClassOrderSlotMaster=1 ";
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and TCOS.StudyTimeWeek=".$SelectDateWeek." ";
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and (datediff(TCOS.ClassOrderSlotDate, '".$SelectDate."')=0 or TCOS.ClassOrderSlotDate is null) ";


/*
$AddSqlWhere = $AddSqlWhere . " and ( 
										(COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectDateWeek." and COS.ClassOrderSlotStartDate is NULL and COS.ClassOrderSlotEndDate is NULL and datediff(CO.ClassOrderStartDate, '".$SelectDate."')<=0 and (datediff(CO.ClassOrderEndDate, '".$SelectDate."')>=0 or CO.ClassOrderEndDate is null) ) 

										or 
										(COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectDateWeek." and datediff(COS.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and COS.ClassOrderSlotEndDate is NULL and datediff(CO.ClassOrderStartDate, '".$SelectDate."')<=0 and (datediff(CO.ClassOrderEndDate, '".$SelectDate."')>=0 or CO.ClassOrderEndDate is null) ) 

										or 
										(COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectDateWeek." and COS.ClassOrderSlotStartDate is NULL and datediff(COS.ClassOrderSlotEndDate, '".$SelectDate."')>=0 and datediff(CO.ClassOrderStartDate, '".$SelectDate."')<=0 and (datediff(CO.ClassOrderEndDate, '".$SelectDate."')>=0 or CO.ClassOrderEndDate is null) ) 

										or 
										(COS.ClassOrderSlotType=1 and COS.StudyTimeWeek=".$SelectDateWeek." and datediff(COS.ClassOrderSlotStartDate, '".$SelectDate."')<=0 and datediff(COS.ClassOrderSlotEndDate, '".$SelectDate."')>=0 and datediff(CO.ClassOrderStartDate, '".$SelectDate."')<=0 and (datediff(CO.ClassOrderEndDate, '".$SelectDate."')>=0 or CO.ClassOrderEndDate is null) ) 
...
$TCOS_AddSqlWhere = $TCOS_AddSqlWhere . " and TCOS.ClassOrderSlotState=1 ";
...
if ($SearchNoSetQnV=="1"){
	if($BookSystemType==0) {
		$AddSqlWhere = $AddSqlWhere . " and ( ( CLS.BookQuizID=0 and CLS.BookVideoID=0 and CLS.BookScanID=0 and CLS.BookRegForReason=0 ) or (CLS.BookQuizID is null and CLS.BookVideoID is null and CLS.BookScanID is null and CLS.BookRegForReason=0 ) )";
	} else if($BookSystemType==1) {
		$AddSqlWhere = $AddSqlWhere . " and ( ( CLS.BookQuizID=0 and CLS.BookVideoID=0 and CLS.BookWebookUnitID=0 and CLS.BookRegForReason=0 ) or (CLS.BookQuizID is null and CLS.BookVideoID is null and CLS.BookWebookUnitID is null and CLS.BookRegForReason=0 ) )";
	}
}
...
$TableClassOrderSlots = "
	select 
		TCOS.* 
	from ClassOrderSlots TCOS 
	where ".$TCOS_AddSqlWhere."
";

//echo $TableClassOrderSlots;
...
$ViewTable = "
	...
	select 
		...
		ifnull((select A.ClassID from Classes A where A.ClassOrderID=COS.ClassOrderID and (A.ClassAttendState=0 or A.ClassAttendState=1 or A.ClassAttendState=2) and A.ClassState=2 and datediff(A.EndDateTime, '".$SelectDate."')<0 order by A.StartDateTime desc limit 0,1), 0) as LastClassID,

		ifnull((select B.AssmtStudentDailyScoreID from Classes A inner join AssmtStudentDailyScores B on A.ClassID=B.ClassID where A.ClassOrderID=COS.ClassOrderID and (A.ClassAttendState=0 or A.ClassAttendState=1 or A.ClassAttendState=2) and A.ClassState=2 and datediff(A.EndDateTime, '".$SelectDate."')<0 order by A.StartDateTime desc limit 0,1), 0) as LastAssmtStudentDailyScoreID,
...
if (date("Y-m-d")==$SelectYear."-".substr("0".$SelectMonth,-2)."-".substr("0".$SelectDay,-2)  ||  $ListSelectResetDate=="1"){
	...
	$Sql3 = "
			...
			where 
				(V.CenterPayType=1 and V.MemberPayType=0) 
				or 
				( 
                ...
                <div class="uk-grid" data-uk-grid-margin="">
					...
					<div class="uk-width-medium-1-10" style="display:;">
                        <div class="uk-margin-small-top">
                            <select id="PageListNum" name="PageListNum" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
								...
								<option value="0" <?if ($PageListNum=="0"){?>selected<?}?>>전체</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="uk-width-medium-2-10">
                        <div class="uk-margin-top uk-text-nowrap">
                            ...
                            <label for="SearchNoSetQnV" class="inline-label">교재/퀴즈/비디오미설정</label>
                        </div>
                    </div>
							...
							<table class="uk-table uk-table-align-vertical">
                                ...
                                <tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										$LastClassID = $Row["LastClassID"];
										$LastAssmtStudentDailyScoreID = $Row["LastAssmtStudentDailyScoreID"];

										$LastStudyClassCount = $Row["LastStudyClassCount"];



										$Sql3 = "select MemberLoginID as CenterLoginID from Members where CenterID=:CenterID and MemberLevelID=12";
										$Stmt3 = $DbConn->prepare($Sql3);
										...
										$Row3 = $Stmt3->fetch();
										$Stmt3 = null;
										$CenterLoginID = $Row3["CenterLoginID"];
										...
										if ($ClassMemberType==1){//1:1 이면....
											...
											while($Row_G = $Stmt_G->fetch()) {
												...
												$LastClassID = $Row_G["LastClassID"];
												$LastAssmtStudentDailyScoreID = $Row_G["LastAssmtStudentDailyScoreID"];
...
function OpenScan(ScanID, ClassID, BookSystemType) {
	...
	if(BookSystemType==0) { // mangoi
		...
		if(ScanID=="") {
			...
			$.post( "../webook/_get_unit_content.php", { content_type:"학생", unit_id:ScanID, api_extension:"Content URL", width:"100%", height: "100%", unit_contents_type:StrContentType, async:false })
			  .done(function( data ) {
	...
	if(CheckResult==true) {
		alert("교재 URL 복사 되었습니다.");
	} else {
		alert("교재 URL 복사가 되지 않았습니다.");
	}
...
function OpenClassReg(ClassOrderID, StartYear, StartMonth, StartDay, StartHour, StartMinute, ClassOrderTimeTypeID, TeacherID, MemberID, ClassMemberType, LastClassID, LastAssmtStudentDailyScoreID, RegType, GroupRowCount, ClassProductID, ClassOrderSlotType, StudyTimeWeek){//RegType 1:등록 2:등록전 연기 
    ...
    $.ajax(url, {
        ...
        success: function (data) {
			...
			<?if ($ListSelectResetDate!=""){?>
				...
				ClassResetBtns = "<a class=\"md-btn md-btn-gray md-btn-mini md-btn-wave-light\" href=\"javascript:OpenResetDateForm("+ClassID+", "+GroupRowCount+", "+ClassMemberType+");\"><?=$수업연기[$LangID]?></a><a class=\"md-btn md-btn-gray md-btn-mini md-btn-wave-light\" href=\"javascript:OpenResetDateForm2("+ClassID+","+ClassOrderID+","+MemberID+","+TeacherID+", "+GroupRowCount+", "+ClassMemberType+");\" style=\"background-color:#f1f1f1;\"><?=$강사변경[$LangID]?></a>";
			
				if (ClassProductID==1 && ClassOrderSlotType==1){
					ClassResetBtns = ClassResetBtns + "<br><a class=\"md-btn md-btn-gray md-btn-mini md-btn-wave-light\" href=\"javascript:OpenResetDateForm3("+ClassID+","+ClassOrderID+","+MemberID+","+TeacherID+", "+GroupRowCount+", "+ClassMemberType+");\" style=\"margin-top:10px;background-color:#C7C7C7;\"><?=$보강등록[$LangID]?></a><a class=\"md-btn md-btn-gray md-btn-mini md-btn-wave-light\" href=\"javascript:OpenResetDateForm4("+ClassID+","+ClassOrderID+","+MemberID+","+TeacherID+", "+GroupRowCount+", "+ClassMemberType+", "+StartYear+","+StartMonth+","+StartDay+", "+StartHour+", "+StartMinute+", "+StudyTimeWeek+");\" style=\"margin-top:10px;background-color:#999999;color:#ffffff;\"><?=$스케줄변경[$LangID]?></a>";
				}
			...
			if (RegType==2){//연기일 경우
				OpenResetDateForm(ClassID, GroupRowCount, ClassMemberType);
			}else if (RegType==3){//강사변경
				OpenResetDateForm2(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType);
			}else if (RegType==4){//보충강의
				OpenResetDateForm3(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType);
			}else if (RegType==5){//스케줄변경
				OpenResetDateForm4(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType, StartYear, StartMonth, StartDay, StartHour, StartMinute, StudyTimeWeek);
			}
...
function CopyScanLink(BookLink, ClassID, Type, MemberLoginID) {
	...
	if(Type==0) { // mangoi
		...
		var StrContentType = "일반교재";
		alert(BookLink);
		$.post( "../webook/_get_unit_content.php", { content_type:"학생", unit_id:BookLink, api_extension:"Content URL", width:"100%", height: "100%", unit_contents_type:StrContentType })
		  .done(function( data ) {
...
function OpenResetDateForm(ClassID, GroupRowCount, ClassMemberType){//연기
	var OpenUrl = "../pop_class_reset_date_form.php?ClassID="+ClassID+"&GroupRowCount="+GroupRowCount+"&ClassMemberType="+ClassMemberType;
...
function OpenResetDateForm2(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType){//강사변경
...
function OpenResetDateForm3(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType){//보강
...
function OpenResetDateForm4(ClassID, ClassOrderID, MemberID, TeacherID, GroupRowCount, ClassMemberType, SetYear, SetMonth, SetDay, SetHour, SetMinute, SetWeek){//보강
...
function OpenClassSh(ClassID, CommonShClassCode, MemberType, MemberName, MemberLoginID){
	...
	if (ClassTimeCheck(ClassID)){
		...
		alert("강의실 입장은 시작시간 전후 10분 이내만 허용합니다.");
	}
...
window.onload = function(){
	...
	CheckNewClassChMsg();
	<?if ($_LINK_ADMIN_LEVEL_ID_==15){?>
		MyInterval = setInterval(CheckNewClassChMsg, 10000);
	<?}?>
}
//수업 자동등록(당일날 처음 열때 등록해 준다)
</script>
...
function SearchSubmit(){
	document.SearchForm.action = "class_list_dev_dev.php";
    document.SearchForm.submit();
}
</script>

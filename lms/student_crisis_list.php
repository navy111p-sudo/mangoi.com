...
$SearchClassOrderEndDateNum2 = isset($_REQUEST["SearchClassOrderEndDateNum2"]) ? $_REQUEST["SearchClassOrderEndDateNum2"] : "";
...
if ($SearchClassOrderEndDateNum==""){
	$SearchClassOrderEndDateNum = "-7";
}
if ($SearchClassOrderEndDateNum2==""){
	$SearchClassOrderEndDateNum2 = "-30";
}
...
if ($SearchClassOrderEndDateNum!="100"){
	...
	(
		( V.CenterPayType=1 and V.MemberPayType=0 and V.CenterStudyEndDate <> '0000-00-00' and V.CenterStudyEndDate <> '' and V.CenterStudyEndDate Is NOT NULL and datediff(V.CenterStudyEndDate, now())<=".$SearchClassOrderEndDateNum." and datediff(V.CenterStudyEndDate, now()) >".$SearchClassOrderEndDateNum2.") 
		or 
		(V.CenterPayType=1 and V.MemberPayType=1 and V.MemberID in (select MemberID from ClassOrders where ClassOrderEndDate <> '' and ClassOrderEndDate IS NOT NULL and datediff(ClassOrderEndDate, now())<=".$SearchClassOrderEndDateNum." and datediff(ClassOrderEndDate, now())>".$SearchClassOrderEndDateNum2." and (ClassOrderState=1 or ClassOrderState=2 or ClassOrderState=4) and ClassProgress=11 and ClassProductID=1 ) ) 
		or 
		(V.CenterPayType=2 and V.MemberID in (select MemberID from ClassOrders where ClassOrderEndDate <> '' and ClassOrderEndDate IS NOT NULL and datediff(ClassOrderEndDate, now())<=".$SearchClassOrderEndDateNum." and datediff(ClassOrderEndDate, now())>".$SearchClassOrderEndDateNum2." and (ClassOrderState=1 or ClassOrderState=2 or ClassOrderState=4) and ClassProgress=11 and ClassProductID=1 ) ) 
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom">종료일자 확인</h3>
		...
		<div class="md-card" style="margin-bottom:10px;">
			<div class="md-card-content">
				<div class="uk-grid" data-uk-grid-margin="">
					...
					<div class="uk-width-medium-1-10">
						<div class="uk-margin-small-top">
							<select id="SearchClassOrderEndDateNum" name="SearchClassOrderEndDateNum" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
								...
								<?for ($ii=-7;$ii>=-29;$ii--){?>
								<option value="<?=$ii?>" <?if ($ii==$SearchClassOrderEndDateNum){?>selected<?}?>>종료 <?=$ii?>일 부터</option>
								<?}?>
							</select>
						</div>
					</div>
					<div class="uk-width-medium-1-10">
						<div class="uk-margin-small-top">
							<select id="SearchClassOrderEndDateNum2" name="SearchClassOrderEndDateNum2" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
								<option value="100"><?=$전체[$LangID]?></option>
								<?for ($ii=-9;$ii>=-50;$ii--){?>
								<option value="<?=$ii?>" <?if ($ii==$SearchClassOrderEndDateNum2){?>selected<?}?>>종료 <?=$ii?>일 까지</option>
								<?}?>
							</select>
						</div>
					</div>
					...
					<div class="uk-width-medium-2-10">
						<label for="SearchText"><?=$추천인아이디[$LangID]?></label>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
							...
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										...
										<th nowrap style="width:8%"><?=$상담[$LangID]?></th>
										<!--
								...
								<tbody>
									...
									<tr id="TrList_<?=$MemberID?>" onclick="SelectList(<?=$MemberID?>);" class="">
										...
										<td class="uk-text-nowrap uk-table-td-center"><a href="student_form.php?ListParam=<?=$ListParam?>&MemberID=<?=$MemberID?>">
											<?=$MemberLoginID?>
												<?if ($_LINK_ADMIN_LEVEL_ID_<=4 || $_LINK_ADMIN_LEVEL_ID_==15 || $_LINK_ADMIN_LEVEL_ID_==12 | $_LINK_ADMIN_LEVEL_ID_==13){?>
													<a href="javascript:OpenMemberPointForm(<?=$MemberID?>);"><i class="material-icons">monetization_on</i></a>
												<?}?>
												<?if ($_LINK_ADMIN_LEVEL_ID_<=4){?>
													<a href="javascript:OpenStudentFormPop(<?=$MemberID?>);"><i class="material-icons">drag_indicator</i></a>
												<?}?>
										</a></td>
										...
										<td class="uk-text-nowrap uk-table-td-center"><a href="javascript:OpenCounselForm(<?=$MemberID?>);"><i class="material-icons" style="font-size:36px;color:blueviolet">contact_phone</i></a></td>
										<!--
						...
						<div class="uk-form-row btns">
							...
							<?if ($_LINK_ADMIN_LEVEL_ID_<=4){?>
							<a type="button" href="javascript:DownloadList()" class="md-btn md-btn-primary two"><?=$현재목록_다운로드[$LangID]?></a>
							<?}?>
							<a type="button" href="javascript:PopUpUploadForm(<?=$_LINK_ADMIN_ID_?>)" class="md-btn md-btn-primary two"><?=$학생목록_일괄_업로드[$LangID]?></a>
							<a type="button" href="javascript:DownloadForm()" class="md-btn md-btn-primary three"><?=$학생목록_샘플폼_다운로드[$LangID]?></a>
                            </div>
                        </div>
...
function DownloadList(){
	document.SearchForm.action = "student_list_excel.php";
...
function SearchSubmit(){
	document.SearchForm.action = "student_crisis_list.php";
	document.SearchForm.submit();
}
</script>

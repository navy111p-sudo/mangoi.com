...
#--------------------------------------------------------------------------------------------------------------------#
if ($SearchDate==3) {    // 월별
        if ( $SearchStandard==4 && $SearchStandardDetail==9 ) {    //본사 > 연간 신규생 비교, 강사    
		} else {
...
#--------------------------------------------------------------------------------------------------------------------#
if ($SearchStandard==3 && $SearchStandardDetail==3) {           // 강사 월별 탈락률을 구하기 위한 이전달 세팅
       
	   if ($SearchEndMonth==1) {
              $BeforYear = $SearchStartYear - 1;
              $BeforDate = $BeforYear . "-12";
       } else {
		      $BeforEndMonth = $SearchEndMonth - 1;
              $BeforDate = $SearchStartYear . "-" . substr("0".$BeforEndMonth,-2);
	   }
}
#--------------------------------------------------------------------------------------------------------------------#
$Search_Parameter = "";
        ...
        <div class="md-card" style="margin-bottom:10px;">
            <div class="md-card-content">
                <div class="uk-grid" data-uk-grid-margin="">
                    ...
                    ?>
                    <div class="uk-width-medium-5-10" style="padding-top:7px;">
                        <a href="javascript:SearchSubmit();" class="md-btn md-btn-primary uk-margin-small-top"><?=$검색[$LangID]?></a>
						<?
						if($SearchStandard=="1" and $SearchStandardDetail=="5" and $SearchStartYear==date("Y") and $SearchEndMonth==date("m")) {
							?>
                            <b style="color:#0000ff;">당월은 월말에 정확한 집계가 잡힙니다!</b>
							<? 
					    } else if ($SearchStandard==3 && $SearchStandardDetail==3 and $SearchStartYear==date("Y") and $SearchEndMonth==date("m")) { // 강사 월별 탈락률
							?>
                            <b style="color:#0000ff;">이전달과 비교하기 때문에 당월은 월말에 정확한 집계가 잡힙니다!</b>
							<? 
						}
						?>
                    </div>
            ...
            $Sql = "select count(*) as TotalResultCount,
                  ...
                  inner join Classes B on A.MemberID=B.MemberID
                        where A.MemberLevelID=19 and 
                              A.MemberState=1 and  
                              B.ClassState=1 and 
                              date_format(B.ClassRegDateTime, '%Y-%m')='".$SearchEndDate."'
                        group by C.CenterName, date_format(B.ClassRegDateTime, '%Y-%m') 
                        order by B.ClassRegDateTime asc";
            $Stmt = $DbConn->prepare($Sql);
            $Stmt->execute();
            $Stmt->setFetchMode(PDO::FETCH_ASSOC);
            while($Row = $Stmt->fetch()) {
            ...
            $Sql = "select count(*) as TotalResultCount,
                  ...
                  inner join Branches    D on C.BranchID=D.BranchID
                        where A.MemberLevelID=19 and 
                              ...
                              iif($SearchStandardDetailSub > 0, " and D.BranchID=$SearchStandardDetailSub","") . " and 
                              date_format(B.ClassRegDateTime, '%Y-%m')='".$SearchEndDate."'
                        group by D.BranchName, date_format(B.ClassRegDateTime, '%Y-%m') 
                        order by B.ClassRegDateTime asc";
            $Stmt = $DbConn->prepare($Sql);
            $Stmt->execute();
            $Stmt->setFetchMode(PDO::FETCH_ASSOC);
...
#====================================================================================================================#
      ...
      #--------------------------------------------------------------------------------------------------------------#
            ...
            #------------------------------------------------------------------------------------------------#
            $Befor_ArrTotalResultCount = [];
            $Sql = "select ifnull(count(*), 0) as TotalResultCountDrawal,
                            T.TeacherName as TeacherName
                        from Members A 
                  inner join Classes  B on A.MemberID=B.MemberID
                  inner join Teachers T on T.TeacherID=B.TeacherID
                        where A.MemberLevelID=19 and 
                              A.MemberState=1 and  
                              B.ClassState=1 and 
                              date_format(B.ClassRegDateTime, '%Y-%m')='".$BeforDate."'
                        group by B.TeacherID, date_format(B.ClassRegDateTime, '%Y-%m') 
                        order by B.ClassRegDateTime asc";
            $Stmt = $DbConn->prepare($Sql);
            $Stmt->execute();
            $Stmt->setFetchMode(PDO::FETCH_ASSOC);
            while($Row = $Stmt->fetch()) {
                 ...
                 $Befor_ArrTotalResultCount[$TeacherName] = $Row["TotalResultCountDrawal"];
            }
            #------------------------------------------------------------------------------------------------#
            $Sql = "select ifnull(count(*), 0) as TotalResultCountDrawal,
                           T.TeacherName as TeacherName
            ...
            while($Row = $Stmt->fetch()) {
                  ...
                  if (isset($Befor_ArrTotalResultCount[$TeacherName]) and $Row["TotalResultCountDrawal"] > 0) {
                        $TotalResultCountDrawal_Ratio = $Befor_ArrTotalResultCount[$TeacherName] - $Row["TotalResultCountDrawal"]; // 지난달학생수 - 이번달 학생수
                        $TotalResultCountDrawal_Ratio = $TotalResultCountDrawal_Ratio / $Befor_ArrTotalResultCount[$TeacherName];  // 결과값 / 지난달 학생수
                        $TotalResultCountDrawal_Ratio = $TotalResultCountDrawal_Ratio * 100;                                         
				  }
				  if ($TotalResultCountDrawal_Ratio > 0) {
                        $ArrTotalResultCount[$TeacherName] = "" . round($TotalResultCountDrawal_Ratio) . "";
				  }
                    ...
                    <div class="uk-width-1-1">
                        <div class="uk-overflow-container">
                            ...
                            if($SearchStandard && $SearchStandardDetail) {
                                  ...
                                  /* 
                                  if($SearchStandard==3 && $SearchStandardDetail==3) {

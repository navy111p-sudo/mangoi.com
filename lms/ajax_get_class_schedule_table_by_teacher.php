...
$AddSqlWhere = "1=1";
$AddSqlWhere = $AddSqlWhere . " and A.TeacherID=$TeacherID "; 
...
$Sql = "select 
				TeacherName,
				TeacherStartHour as StartHour, 
				TeacherEndHour as EndHour,
				TeacherBlock80Min
		from Teachers A 
		where ".$AddSqlWhere." ";
$Stmt = $DbConn->prepare($Sql);
...
for ($WeekDayNum=0;$WeekDayNum<=6;$WeekDayNum++){
	for ($HourNum=$StartHour;$HourNum<=$EndHour;$HourNum++){//전체를 초기화 해준다.
...
for ($HourNum=$StartHour;$HourNum<$EndHour;$HourNum++){//해당되는 리스트만 가져온다.
	...
	if ($HourListCount % 9 == 0){
		...
		$ScheduleTable .= "<th nowrap colspan=\"7\" class=\"TdTeacherName_".$TeacherNum."\" style=\"width:70px;\">".$TeacherName." ".$StrBlock80Min."</th>";
		...
		for ($WeekDayIndex=0;$WeekDayIndex<7;$WeekDayIndex++){
			...
			if ($_LINK_ADMIN_LEVEL_ID_>4){// 관리자가 아니면 General / Homebase 만 나오게 한다.
				$AddSqlWhere_TeacherGroups = " and (TEA.TeacherGroupID=4 or TEA.TeacherGroupID=9) ";
			} else {
				$AddSqlWhere_TeacherGroups = "";
			}

			$SearchDate = date("Y-m-d", strtotime($ClassWeekStartDate." + ".($WeekDayIndex)." days"));
			$WeekDayNum = date("w", strtotime($ClassWeekStartDate." + ".($WeekDayIndex)." days"));
			...
			$TeacherClassCount = "$ClassCount/$MinuteCount";



			//for ($WeekDayNum=0;$WeekDayNum<=6;$WeekDayNum++){
				//if ($EduCenterHoliday[$WeekDayNum]==0) {
					$WeekDayIndexDate = date("d", strtotime($ClassWeekStartDate." + ".($WeekDayIndex)." days"));
					$ScheduleTable .= "<th nowrap class=\"TdWeekName_".$TeacherNum."_".$WeekDayIndex."\">".$WeekDayIndexDate."일(".$ArrWeekDayStr[$WeekDayIndex].") - ".$TeacherClassCount."</th>";
				//}
			//}
		}
	...
	for ($MinuteNum=0;$MinuteNum<=50;$MinuteNum=$MinuteNum+10){
		...
		for ($WeekDayNumIndex=1;$WeekDayNumIndex<=7;$WeekDayNumIndex++){
			$SearchDateDay = date("Y-m-d", strtotime($ClassWeekStartDate." + ".($WeekDayNumIndex-1)." days"));
			$SearchDate = $SearchDateDay;
			$WeekDayNum = $WeekDayNumIndex - 1;
			...
			//=====================================================  BBBB
					...
					//$ForceBlock = 0;
					if ($SlotBreakEvent ==0 && $TeacherBlock80Min==1){//80분 연속수업인지 검색

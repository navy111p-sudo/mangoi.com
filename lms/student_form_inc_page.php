	...
	<div class="uk-grid" data-uk-grid-margin>
			...
			<div class="md-card">
				...
				<div class="user_content">
					...
					<ul id="user_edit_tabs" class="uk-tab" data-uk-tab="{connect:'#user_edit_tabs_content'}" style="display:<?if ($MemberID==""){?>none<?}?>;">
						...
						<li <?if ($PageTabID=="6"){?>class="uk-active"<?}?>><a href="#"><?=$상담내역[$LangID]?></a></li>
						<li <?if ($PageTabID=="7"){?>class="uk-active"<?}?>><a href="#"><?=$메시지내역[$LangID]?></a></li>
						<li <?if ($PageTabID=="8"){?>class="uk-active"<?}?>><a href="#"><?=$포인트내역[$LangID]?></a></li>
						<li <?if ($PageTabID=="11"){?>class="uk-active"<?}?>><a href="#"><?=$결제내역[$LangID]?></a></li>
						<li <?if ($PageTabID=="9"){?>class="uk-active"<?}?>><a href="#"><?=$정기평가보고서[$LangID]?></a></li>
						<li <?if ($PageTabID=="10"){?>class="uk-active"<?}?>><a href="#"><?=$레벨테스트[$LangID]?></a></li>
						<li <?if ($PageTabID=="12"){?>class="uk-active"<?}?>><a href="#"><?=$교재구매바구니[$LangID]?></a></li>
								...
								<div class="uk-grid" data-uk-grid-margin>
									<div class="uk-width-medium-4-10" style="padding-top:7px;display:<?if ($HideCenterID==1){?>none<?}?>;">
										<select id="CenterID" name="CenterID" class="uk-width-1-1" data-md-select2 data-allow-clear="true" data-placeholder="대리점선택" style="width:100%;"/>
											<!--<option value="0">소속 대리점 없음</option>-->
											<?
											$Sql2 = "select 
															A.*,
															B.BranchName,
						...
						<li>
							...
							<table class="uk-table uk-table-align-vertical">
								...
								<tr>
									...
									<td class="uk-text-nowrap"><?=$ProductSellerName?></td>
									...
									<td class="uk-text-nowrap uk-table-td-center"><a href="javascript:OpenProductOrderCartForm(<?=$ProductOrderCartID?>);"><?=$상세보기[$LangID]?></a></td>
								</tr>
								<?php
									$ListCount ++;
								}
							...
							<div class="uk-form-row" style="text-align:center;">
								<a type="button" href="javascript:OpenProductOrderCartForm('')" class="md-btn md-btn-primary"><?=$신규등록[$LangID]?></a>
							</div>
						</li>

						<li>
							<!--스케쥴-->
							<iframe src="../pop_study_calendar.php?MemberID=<?=$MemberID?>&IframeMode=1" scrolling="no" frameborder="1" id="iframe_calendar" height="600" width="100%"></iframe>
						</li>
						<?
						}
						?>
		...
		<div class="uk-width-large-3-10">
			<div class="md-card">
				...
				<?if($MemberState!=3) { ?>
					<h3 class="heading_c uk-margin-medium-bottom"><?=$학생상태설정[$LangID]?></h3>
					...
					<div class="uk-form-row" style="display:none;">
						<input type="checkbox" id="ForceUseClassIn" name="ForceUseClassIn" value="1" <?php if ($ForceUseClassIn==1) { echo "checked";}?> data-switchery/>
						<label for="ForceUseClassIn" class="inline-label"><?=$클래스인만_사용[$LangID]?>/label>
					</div>
					<hr class="md-hr" style="display:none;">

					<div class="uk-form-row" style="display:<?if ($_LINK_ADMIN_LEVEL_ID_>7){?>none<?}?>;">
						<label for="MemberPricePerTime"><?=$수업료_10분[$LangID]?></label>
						<input type="number" id="MemberPricePerTime" name="MemberPricePerTime" value="<?=$MemberPricePerTime?>" class="md-input label-fixed allownumericwithoutdecimal"/>
						※ 0 으로 입력하면 결제시 대리점 수강료가 적용됩니다.
					</div>
					...
					<div class="uk-form-row" style="display:<?if ($_LINK_ADMIN_LEVEL_ID_>7){?>none<?}?>;">
						<span class="icheck-inline">
							<input type="radio" id="MemberPayType0" name="MemberPayType" <?php if ($MemberPayType==0) { echo "checked";}?> value="0" data-md-icheck/>
							<label for="MemberPayType0" class="inline-label"><?=$단체결제[$LangID]?></label>
						</span>
						<span class="icheck-inline">
							<input type="radio" id="MemberPayType1" name="MemberPayType" <?php if ($MemberPayType==1) { echo "checked";}?> value="1" data-md-icheck/>
							<label for="MemberPayType1" class="inline-label"><?=$개인결제[$LangID]?></label>
						</span>
						<div style="color:#ff0000;">※ 단체결제 -> 개인결제로 변경시 수강신청관리에서 종료일을 설정해 주셔야 합니다.</div>
					</div>
					<hr class="md-hr" style="display:<?if ($_LINK_ADMIN_LEVEL_ID_>7){?>none<?}?>;">
					<?}else{?>
					<input type="hidden" name="MemberPayType" value="<?=$MemberPayType?>">
					<?}?>
					<div class="uk-form-row" style="display:<?if ($_LINK_ADMIN_LEVEL_ID_>4){?>none<?}?>;">
						<span class="icheck-inline">
							<input type="radio" id="MemberState1" name="MemberState" <?php if ($MemberState==1) { echo "checked";}?> value="1" data-md-icheck/>
							<label for="MemberState1" class="inline-label"><?=$정상[$LangID]?></label>
						</span>
						<span class="icheck-inline">
							<input type="radio" id="MemberState2" name="MemberState" <?php if ($MemberState==2) { echo "checked";}?> value="2" data-md-icheck/>
							<label for="MemberState2" class="inline-label"><?=$휴면[$LangID]?></label>
						</span>
						<span class="icheck-inline">
							<input type="radio" id="MemberState3" name="MemberState" <?php if ($MemberState==3) { echo "checked";}?> value="3" data-md-icheck/>
							<label for="MemberState3" class="inline-label"><?=$탈퇴[$LangID]?></label>
						</span>
					</div>
					<span style="color: #FF0000;display:<?if ($_LINK_ADMIN_LEVEL_ID_>4){?>none<?}?>;">탈퇴 했을 경우에는 수정 및 복구가 불가합니다.</span>
					<hr class="md-hr" style="display:<?if ($_LINK_ADMIN_LEVEL_ID_>4){?>none<?}?>;">
				...
				<? } else { ?>
					<div class="uk-form-row" style="">
						<span style="color:#FF0000">이 계정은 탈퇴된 계정으로 수정, 복구가 불가합니다.</span>
					</div>
				<? } ?>
			</div>
		</div>
...
function OpenProductOrderCartForm(ProductOrderCartID){
	...
	$.colorbox({	
		...
		,onClosed:function(){
			//location.reload();
			<?if ($PageType=="POP"){?>
			location.href='student_form_pop.php?MemberID=<?=$MemberID?>&ListParam=<?=$ListParam?>&PageTabID=12';
			<?}else{?>
			location.href='student_form.php?MemberID=<?=$MemberID?>&ListParam=<?=$ListParam?>&PageTabID=12';
			<?}?>
		}

...
$Sql = "SELECT count(*) as TotalRowCount
            ...
            where A.Hr_CompetencyIndicatorCate1State=1 and (B.Hr_CompetencyIndicatorCate2State=1 or B.Hr_CompetencyIndicatorCate2State is null) 
                AND C.Hr_CompetencyIndicatorState = 1 
                and B.Hr_CompetencyIndicatorCate2ID > 0 and T.Hr_OrganTask2ID=".$OrganTask2ID;
            ...
            <div class="md-card-content">
                <div class="uk-grid" data-uk-grid-margin>
                    ...
                    ?>
                            ...
                            <?
                            #-------------------------------------------------------------------------------------------------------------#
                            // 평가자와 평가 대상자의 관계(상사, 부하, 동료)를 가지고 온다
                            $Sql = "SELECT Hr_EvaluationCompetencyMemberType 
                                    FROM Hr_EvaluationCompetencyMembers
                                    WHERE MemberID = :MemberID AND Hr_EvaluationCompetencyMemberID = :Hr_EvaluationCompetencyMemberID
                                        AND Hr_EvaluationID = :Hr_EvaluationID";
                            $Stmt = $DbConn->prepare($Sql);
                            ...
                            $Sql = "SELECT 
                                    ...
                                    FROM Hr_CompetencyIndicatorCate1 A 
										...
										left outer join Hr_Staff_Compentency CP       on CP.Hr_CompetencyIndicatorID=C.Hr_CompetencyIndicatorID and 
										                                                 CP.MyMemberID=".$My_MemberID." and 
																						 CP.MemberID=".$MemberID." and
																						 CP.Hr_EvaluationID=".$SearchState." and 
																						 CP.Hr_OrganTask1ID=".$OrganTask1ID." and 
																						 CP.Hr_OrganTask2ID=".$OrganTask2ID." 
                                    ...
                                    WHERE A.Hr_CompetencyIndicatorCate1State=1 and (B.Hr_CompetencyIndicatorCate2State=1 or B.Hr_CompetencyIndicatorCate2State is null) and
                                          ...
                                          AND C.Hr_MemberType".$Hr_EvaluationCompetencyMemberType." = 1
                                          AND C.Hr_CompetencyIndicatorState = 1 
							...
							if ($COMP_State==9 and $COMP_UseYN=='Y') {
                                    ...
                                    <tr>
                                        ...
                                        <td class="uk-text-nowrap uk-table-td-center" style="text-align:center; color:#ff0000;" colspan="5"><?=number_format($CPPoint_Avg,2)?></td>
                                    </tr>
                            <?
							}
                            $Stmt = null;
                            ...
                            <div class="uk-form-row" style="text-align:center; margin-top:20px;">
                                <a type="button" href="javascript:Competency_Save(1)" class="md-btn md-btn-primary" style="background:#CACACA; color:#5B5B5B;"><?=$임시저장[$LangID]?></a>
                                <a type="button" href="javascript:Competency_Save(9)" class="md-btn md-btn-primary"><?=$평가제출[$LangID]?></a>
                            </div>
							<?
							} else {
                            ?>
                            <div class="uk-form-row" style="text-align:center; padding:20px; color:#BCBCBC; font-size:1.5em;">
                                 <?=$MemberName?> <?=$의_평가가_완료_되었습니다[$LangID]?>
                            </div>
							<?
							}
							?>
...
function Competency_Save(s) {
	   ...
	   var MemberName   = document.RegForm.MemberName.value;
       var OrganTask1ID = document.RegForm.OrganTask1ID.value;
       var OrganTask2ID = document.RegForm.OrganTask2ID.value;
       var line_cnt     = document.RegForm.line_cnt.value;

       for(var k = 1; k <= line_cnt; k++) {

              if (!document.getElementById("CPoint1_"+k).checked && 
				  !document.getElementById("CPoint2_"+k).checked && 
				  !document.getElementById("CPoint3_"+k).checked && 
				  !document.getElementById("CPoint4_"+k).checked && 
				  !document.getElementById("CPoint5_"+k).checked ) {
                     alert(k + "<?=$번_평가를_선택_하세요[$LangID]?>");
                     return;
              }

       }
	   /* 
       if (!document.getElementById("COMP_COMMENT").value) {
              alert("최종평가의견을 입력 하세요!");
              document.getElementById("COMP_COMMENT").focus(); 
              return;
       }
	   */
       var COMP_COMMENT = document.getElementById("COMP_COMMENT").value;

       //-----------------------------------------------------------------------------------------------------------------//
       if (s == 1) {       // 임시저장
       //-----------------------------------------------------------------------------------------------------------------//
             UIkit.modal.confirm(
                 '<?=$역량평가를_임시_저장하시겠습니까[$LangID]?>', 
                 function(){ 

                       document.RegForm.action = "hr_staff_competency_action.php?Compsw=" + s;
       ...
       //-----------------------------------------------------------------------------------------------------------------//
             UIkit.modal.confirm(
                 '<?=$역량평가를_제출_하시겠습니까[$LangID]?><br>※ <?=$한번_제출된_자료는_수정할_수_없습니다[$LangID]?> ※', 
                 function(){ 

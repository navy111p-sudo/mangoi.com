...
if ($SearchState!="100"){
	...
	$AddSqlWhere = $AddSqlWhere . " and A.OnlineSiteState=$SearchState ";
}
$AddSqlWhere = $AddSqlWhere . " and A.OnlineSiteState<>0 ";
$AddSqlWhere = $AddSqlWhere . " and B.FranchiseState<>0 ";

if ($SearchText!=""){
	$ListParam = $ListParam . "&SearchText=" . $SearchText;
	$AddSqlWhere = $AddSqlWhere . " and (A.OnlineSiteName like '%".$SearchText."%' or A.OnlineSiteManagerName like '%".$SearchText."%') ";
}
...
$StudentCountSql = ",(
			...
			where AA.MemberLevelID=19 and AA.MemberState<>0 and BB.OnlineSiteID=A.OnlineSiteID 
			) as StudentCount 
			";
...
$Sql = "
		select 
			...
			B.FranchiseName,
			ifnull((select count(*) from Centers where OnlineSiteID=A.OnlineSiteID and CenterState<>0),0) as CenterCount
			".$StudentCountSql."
		from OnlineSites A 
			inner join Franchises B on A.FranchiseID=B.FranchiseID 
		where ".$AddSqlWhere." 
		order by A.OnlineSiteOrder desc limit $StartRowNum, $PageListNum";
$Stmt = $DbConn->prepare($Sql);
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$독립사이트관리[$LangID]?></h3>
			...
			<div class="md-card-content">
				<div class="uk-grid" data-uk-grid-margin="">
					...
					<div class="uk-width-medium-2-10">
						<label for="SearchText"><?=$사이트명_또는_관리자명[$LangID]?></label>
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
						<div class="uk-overflow-container">
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										<th nowrap>No</th>
										<th nowrap><?=$사이트명[$LangID]?></th>
										
										<th nowrap><?=$도메인[$LangID]?></th>
								...
								<tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										if ($OnlineSiteState==1){
											$StrOnlineSiteState = "<span class=\"ListState_1\">운영중</span>";
										}else if ($OnlineSiteState==2){
											$StrOnlineSiteState = "<span class=\"ListState_2\">미운영</span>";
										}

									?>
									<tr>
										<td class="uk-text-nowrap uk-table-td-center"><?=$ListNumber?></td>
										<td class="uk-text-nowrap uk-table-td-center"><a href="online_site_form.php?ListParam=<?=$ListParam?>&OnlineSiteID=<?=$OnlineSiteID?>"><?=$OnlineSiteName?></a></td>
										
										<td class="uk-text-nowrap uk-table-td-center"><?=$OnlineSiteDomain?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$OnlineSiteManagerName?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=number_format($StudentCount,0)?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$OnlineSitePhone1?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$OnlineSitePhone2?></td>
										...
										<td class="uk-text-nowrap uk-table-td-center"><?=$StrOnlineSiteState?></td>
									</tr>
									<?php
										$ListCount ++;
									}
						...
						<div class="uk-form-row" style="text-align:center;">
							<a type="button" href="online_site_form.php?ListParam=<?=$ListParam?>" class="md-btn md-btn-primary"><?=$신규등록[$LangID]?></a>
						</div>
...
function SearchSubmit(){
	document.SearchForm.action = "online_site_list.php";

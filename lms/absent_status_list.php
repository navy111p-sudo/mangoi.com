...
if ($SearchState!="100"){
	...
	//$AddSqlWhere = $AddSqlWhere . " and A.CounselState=$SearchState ";
}
//$AddSqlWhere = $AddSqlWhere . " and A.CounselState<>0 ";
...
$AddSqlWhere = $AddSqlWhere . " and A.LastClassAttendState=3 ";
$AddSqlWhere = $AddSqlWhere . " and datediff(A.StartDateTime, '".$SearchStartDate."')>=0 ";
$AddSqlWhere = $AddSqlWhere . " and datediff(A.StartDateTime, '".$SearchEndDate."')<=0 ";
...
$ViewTable = "
	...
	select
		...
		TeacherID,  
		ClassAttendState, 
		ContinuityClassAttendState,
		(select ClassAttendState from Classes WHERE MemberID=AAAA.MemberID and datediff(StartDateTime, AAAA.StartDateTime)=0 order by StartDateTime desc limit 0, 1) as LastClassAttendState 
	...
	from ( 
			select  
				...
				TeacherID, 
				ClassAttendState, 
				IF( @PrevClassAttendState = ClassAttendState, @ContinuityClassAttendState:= @ContinuityClassAttendState + 1, @ContinuityClassAttendState:= 1) as ContinuityClassAttendState, 
				@PrevClassAttendState := ClassAttendState 
			from Classes 
			order by MemberID ASC, StartDateTime ASC 
		) AAAA 
";
...
$Sql = "select 
		...
		from ($ViewTable) A 
			...
			inner join Members H on A.TeacherID=H.TeacherID and H.MemberLevelID=15  
		where ".$AddSqlWhere." ";
...
$Sql = "
		...
		from ($ViewTable) A 
			...
			inner join Members H on A.TeacherID=H.TeacherID and H.MemberLevelID=15  
		where ".$AddSqlWhere." 
		order by A.ContinuityClassAttendState desc";// limit $StartRowNum, $PageListNum";
$Stmt = $DbConn->prepare($Sql);
$Stmt->execute();
$Stmt->setFetchMode(PDO::FETCH_ASSOC);
?>
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom">연속결석</h3>
		...
		<div class="md-card" style="margin-bottom:10px;">
			<div class="md-card-content">
				<div class="uk-grid" data-uk-grid-margin="">
							...
							while($Row2 = $Stmt2->fetch()) {
								...
								if ($OldSelectCenterState!=$SelectCenterState){
									...
									if ($SelectCenterState==1){
										echo "<optgroup label=\"$대리점[$LangID]( $운영중[$LangID])\">";
									}else if ($SelectCenterState==2){
										echo "<optgroup label=\"$대리점[$LangID]( $미운영[$LangID])\">";
									}
								}
					...
					<div class="uk-width-medium-2-10" style="padding-top:7px; display: inline-block; float: left; ">
						<label for="uk_dp_start"><?=$기준일[$LangID]?></label>
					...
					<div class="uk-width-medium-2-10" style="padding-top:7px; display: none; float: right;">
				...
				<div class="uk-grid" data-uk-grid-margin>
					<div class="uk-width-1-1">
						<div class="uk-overflow-container">
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										<th nowrap>No</th>
										<th nowrap><?=$날짜_기준일[$LangID]?></th>
										<th nowrap><?=$지사[$LangID]?></th>
										<th nowrap><?=$대리점[$LangID]?></th>
										<th nowrap><?=$강사명_기준일[$LangID]?></th>
										<th nowrap><?=$학생명[$LangID]?></th>
										<th nowrap><?=$아이디[$LangID]?></th>
										<th nowrap><?=$연속결석[$LangID]?></th>
									</tr>
								</thead> 
								<tbody>
									...
									while($Row = $Stmt->fetch()) {
										...
										$ContinuityClassAttendState = $Row["ContinuityClassAttendState"];

										$StrStartDateTime = substr($StartDateTime, 0, 10);
							
									?>
									<tr>
										<td class="uk-text-nowrap uk-table-td-center"><?=$ListNumber?></td>
										<td class="uk-text-nowrap"><?=$StrStartDateTime?></td>
										...
										<td class="uk-text-nowrap uk-table-td-center"><?=$ContinuityClassAttendState?></td>
						...
						<div class="uk-form-row" style="text-align:center;display:none;">
							<a type="button" href="counsel_form.php?ListParam=<?=$ListParam?>" class="md-btn md-btn-primary"><?=$신규등록[$LangID]?></a>
						</div>
...
function OpenCounselForm(CounselID){
	openurl = "counsel_form.php?CounselID="+CounselID;
	$.colorbox({	
		href:openurl
		,width:"95%" 
		,height:"95%"
...
function SearchSubmit(){
	document.SearchForm.action = "absent_status_list.php";

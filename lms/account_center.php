...
if ($SearchYear!=""){
	...
	$AddSqlWhere2 = $AddSqlWhere2 . " and date_format(AAA.ClassOrderPayPaymentDateTime, '%Y')='".$SearchYear."' ";
}

if ($SearchMonth!=""){
	$ListParam = $ListParam . "&SearchMonth=" . $SearchMonth;
	$AddSqlWhere2 = $AddSqlWhere2 . " and date_format(AAA.ClassOrderPayPaymentDateTime, '%m')='".substr("0".$SearchMonth,-2)."' ";
}
				...
				<div class="uk-grid" data-uk-grid-margin="">
					...
					<div class="uk-width-medium-1-10" style="padding-top:7px;display:<?if ($HideSearchBranchGroupID==1){?>none<?}?>;">
						<select id="SearchYear" name="SearchYear" class="uk-width-1-1" onchange="SearchSubmit()" data-md-select2 data-allow-clear="true" data-placeholder="<?=$년도선택[$LangID]?>" style="width:100%;"/>
							<option value="">년도선택<?=$대리점정산[$LangID]?></option>
							<?
					...
					<div class="uk-width-medium-1-10">
						<div class="uk-margin-small-top">
							<select id="SearchOrder" name="SearchOrder" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
								<option value="1" <?if ($SearchOrder=="1"){?>selected<?}?>><?=$총매출_순위[$LangID]?></option>
								<option value="2" <?if ($SearchOrder=="2"){?>selected<?}?>><?=$커미션_순위[$LangID]?></option>
								<option value="3" <?if ($SearchOrder=="3"){?>selected<?}?>><?=$학생수_순위[$LangID]?></option>
							</select>
						</div>
					</div>
					-->
							...
							<table class="uk-table uk-table-align-vertical">
								<thead>
									<tr>
										...
										<?}?>
										<th nowrap><a href="javascript:SetOrderList(4);" ><?=$총매출[$LangID]?> <?if ($CellOrder=="1" && $CellID=="4"){?>▼<?} else if($CellOrder=="2" && $CellID=="4") {?>▲<?}?></a></th>
										<th nowrap><a href="javascript:SetOrderList(5);" ><?=$커미션_3점3_제외[$LangID]?><?if ($CellOrder=="1" && $CellID=="5"){?>▼<?} else if($CellOrder=="2" && $CellID=="5") {?>▲<?}?></a></th>
									</tr>
								</thead>
								<tbody>
									...
									<tr style="background-color:#f1f1f1;">
										<td class="uk-text-nowrap uk-table-td-center" colspan="<?if ($_LINK_ADMIN_LEVEL_ID_<6){?>9<?}else{?>4<?}?>"><?=$합계[$LangID]?></td>
									...
									<tr style="background-color:#f1f1f1;">
										<td class="uk-text-nowrap uk-table-td-center" colspan="<?if ($_LINK_ADMIN_LEVEL_ID_<6){?>9<?}else{?>4<?}?>"><?=$평균[$LangID]?></td>
...
function SearchSubmit(){
	document.SearchForm.action = "account_center.php";

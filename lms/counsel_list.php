...
$Sql = "
		...
		from Counsels A
			...
			left outer join Members H on A.RegMemberID=H.MemberID 
		where ".$AddSqlWhere." 
		order by A.CounselRegDateTime desc limit $StartRowNum, $PageListNum";
$Stmt = $DbConn->prepare($Sql);
...
<div id="page_content">
	<div id="page_content_inner">
		...
		<h3 class="heading_b uk-margin-bottom"><?=$상담관리[$LangID]?></h3>
							...
							while($Row2 = $Stmt2->fetch()) {
								...
								if ($OldSelectFranchiseState!=$SelectFranchiseState){
									...
									if ($SelectFranchiseState==1){
										echo "<optgroup label=\"프랜차이즈( $운영중[$LangID])\">";
									}else if ($SelectFranchiseState==2){
										echo "<optgroup label=\"프랜차이즈( $미운영[$LangID])\">";
									}
								} 
							...
							<table class="uk-table uk-table-align-vertical">
								...
								<tbody>
									...
									<tr>
										...
										<td class="uk-text-nowrap"><a href="javascript:OpenCounselForm(<?=$CounselID?>);"><?=$CounselTitle?></a></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$CounselDate?> <?=$CounselTime?></td>
										<td class="uk-text-nowrap uk-table-td-center"><?=$MemberWriteName?></td>
...
function SearchSubmit(){
	document.SearchForm.action = "counsel_list.php";

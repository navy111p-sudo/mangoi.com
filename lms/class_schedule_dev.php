...
<!-- --------------------------------------------------------------- AA -->
<h3 class="heading_b uk-margin-bottom" style="margin-top:-30px;">전체 스케줄관리</h3>
			...
			<div class="uk-width-medium-2-10">
				<div class="uk-margin-small-top">
					<select id="SearchType" name="SearchType" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
						<option value="2" <?if ($SearchType=="2"){?>selected<?}?>>임시수업 배정기준</option>
						<option value="1" <?if ($SearchType=="1"){?>selected<?}?>>정규수업 배정기준</option>
					</select>
				</div>
			</div>			
			...
			<div class="uk-width-medium-2-10" style="padding-top:7px;display:<?if ($HideSearchFranchiseID==1){?>none<?}?>;">
				<select id="SearchTeacherGroupID" name="SearchTeacherGroupID" class="uk-width-1-1" onchange="SearchSubmit()" data-md-select2 data-allow-clear="true" data-placeholder="<?=$강사그룹선택[$LangID]?>" style="width:100%;"/>
					...
					$Sql2 = "select 
							...
							where A.TeacherGroupState<>0 and A.TeacherGroupView=1 and A.EduCenterID=$EduCenterID
			...
			<div class="uk-width-medium-2-10" style="display:none;">
				<div class="uk-margin-small-top">
					<select id="SearchState" name="SearchState" onchange="SearchSubmit()" data-md-selectize  data-md-selectize-bottom>
						<option value="100" <?if ($SearchState=="100"){?>selected<?}?>>전 체</option>
						<option value="1" <?if ($SearchState=="1"){?>selected<?}?>>활동중</option>
						<option value="2" <?if ($SearchState=="2"){?>selected<?}?>>미활동</option>
					</select>
				</div>
			</div>
...
function SearchSubmit(){
	document.SearchForm.action = "class_schedule_dev.php";
...
function GetScheduleTable(){
	...
	url = "ajax_get_class_schedule_table_by_day_dev.php";

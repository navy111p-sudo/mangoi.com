...
<div class="right">
	<div class="content">
		...
		<div class="box">
			...
			$add_sql_select .= ", (select SubName from Subs where SubID=A.SubID) as SubName ";

			$Sql = "select A.* ".$add_sql_select." from Boards A where ".$AddSqlWhere." order by A.SubID desc limit $StartRowNum, $PageListNum";
			$Stmt = $DbConn->prepare($Sql);
			$Stmt->execute();
			$Stmt->setFetchMode(PDO::FETCH_ASSOC);
			  ...
			  <tr>
				...
				?>
				  <tr>
					<td><?=$ListNumber?></td>
					<td><?=$Row["SubName"]?></td>
					<td class="subject"><?=$Row["BoardCode"]?></td>
					<td class="subject"><a href="boardset_form.php?ListParam=<?=$ListParam?>&BoardID=<?=$Row["BoardID"]?>"><?=$Row["BoardName"]?></a></td>

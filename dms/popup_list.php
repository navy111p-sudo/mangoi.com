...
<div class="right">
	<div class="content">
		...
		<div class="box">
			...
			$Sql = "select count(*) as TotalRowCount from Popups where ".$AddSqlWhere." ";
			...
			$Sql = "select * from Popups  where ".$AddSqlWhere." limit $StartRowNum, $PageListNum";
			$Stmt = $DbConn->prepare($Sql);
			$Stmt->execute();
			$Stmt->setFetchMode(PDO::FETCH_ASSOC);
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
			  <tr>
				...
				?>
				  <tr>
					<td><?=$ListNumber?></td>
					<td class="subject"><a href="popup_form.php?ListParam=<?=$ListParam?>&popupid=<?=$Row["PopupID"]?>"><?=$Row["PopupName"]?></a></td>
					<td class="subject"><?=$Row["PopupTitle"]?></td>
					<td><?=$Row["PopupStartDateNum"]?></td>
					<td><?=$Row["PopupEndDateNum"]?></td>
					<td><a href="javascript:PreviewPopup(<?=$Row["PopupID"]?>,<?=$Row["PopupTop"]?>,<?=$Row["PopupLeft"]?>,<?=$Row["PopupWidth"]?>,<?=$Row["PopupHeight"]?>)">미리보기</a></td>
					<td><?=$str_popup_state?></td>
				  </tr>
				<?php
					$ListCount ++;
				}

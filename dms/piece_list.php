	...
	<div class="content">
		...
		<div class="box">
			...
			if ($SearchState!="2"){
				...
				$AddSqlWhere = $AddSqlWhere . " and A.PieceState=$SearchState ";
			}

			if ($SearchText!=""){
				$ListParam = $ListParam . "&SearchText=" . $SearchText;
				$AddSqlWhere = $AddSqlWhere . " and A.PieceName like '%".$SearchText."%' ";
			}
			...
			$Sql = "select count(*) as TotalRowCount from Pieces A where ".$AddSqlWhere." ";
			...
			$Sql = "select A.* from Pieces A where ".$AddSqlWhere." order by A.PieceName asc limit $StartRowNum, $PageListNum";
			$Stmt = $DbConn->prepare($Sql);
			$Stmt->execute();
			$Stmt->setFetchMode(PDO::FETCH_ASSOC);
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
			  <tr>
				...
				?>
				  <tr>
					<td><?=$ListNumber?></td>
					<td class="subject"><a href="piece_form.php?ListParam=<?=$ListParam?>&PieceID=<?=$Row["PieceID"]?>"><?=$Row["PieceCode"]?></a></td>
					<td class="subject"><a href="piece_form.php?ListParam=<?=$ListParam?>&PieceID=<?=$Row["PieceID"]?>"><?=$Row["PieceName"]?></a></td>

...
$AlertMsg = isset($_REQUEST["AlertMsg"]) ? $_REQUEST["AlertMsg"] : "";
...
if ($TrnLanguageID!=0){
	...
	$Sql = "select 
			...
			where A.TrnCollectUrlID=:TrnCollectUrlID and A.TrnCollectTextState=1 and A.TrnCollectText=:TrnCollectText order by A.TrnCollectTextOrder desc limit 0, 1 ";
	$Stmt = $DbConn->prepare($Sql);
	$Stmt->bindParam(':TrnCollectUrlID', $TrnCollectUrlID);
	$Stmt->bindParam(':TrnCollectText', $AlertMsg);
	$Stmt->execute();
	$Stmt->setFetchMode(PDO::FETCH_ASSOC);
	$Row = $Stmt->fetch();
	$Stmt = null;
	...
	if ($TrnCollectTextID){
		...
		if ($TrnTranslationText!=""){
			$TrnAlertMsg = $TrnTranslationText;
		}else{
...
$ArrValue["TrnAlertMsg"] = $TrnAlertMsg;
